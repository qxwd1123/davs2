...
enum davs2_ret_e {
  ...
  DAVS2_GOT_HEADER = 2, /* Decoding get sequence header, always obtained before
                           DAVS2_GOT_FRAME */
  DAVS2_END = 3, /* Decoding ended: no more bit-stream to decode and no more
                    frames to output */
};
...
typedef struct davs2_seq_info_t {
  ...
  uint32_t
      progressive; /* progressive sequence (0: interlace, 1: progressive) */
...
typedef struct davs2_picture_t {
  ...
  int strides[3];      /* number of bytes in one line are stored continuously in
                          memory */
...
typedef struct davs2_param_t {
  ...
  int info_level; /* only output information which is no less then this level
                     (davs2_log_level_e). 0: All; 1: no debug info; 2: only
                     warning and errors; 3: only errors */
 ...
 * Parameters :
 *       [in] : decoder   - pointer to the AVS2 decoder handler
 *      [out] : headerset - pointer to output common frame information (would
 * always appear before frame output) [out] : out_frame - pointer to output
 * frame information Return     : see definition of davs2_ret_e
 * ---------------------------------------------------------------------------
 */
