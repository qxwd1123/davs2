...
davs2_input_param_t inputparam = { NULL, NULL, NULL, NULL, 0, 0, 0, 0 };
...
/* ---------------------------------------------------------------------------
 */
static void output_decoded_frame(davs2_picture_t *pic,
  ...
  if (pic == NULL || ret_type == DAVS2_GOT_HEADER) {
    show_message(CONSOLE_GREEN,
                 "  Sequence size: %dx%d, %d/%d-bit %.3lf Hz. ProfileLevel: "
                 "0x%x-0x%x\n\n",
                 headerset->width, headerset->height,
                 headerset->internal_bit_depth, headerset->output_bit_depth,
                 headerset->frame_rate, headerset->profile_id,
                 headerset->level_id);

    if (inputparam.b_y4m) {
      static const int FRAME_RATE[9][2] = { { 0, 1 },  // invalid
                                            { 24000, 1001 }, { 24, 1 },
                                            { 25, 1 },       { 30000, 1001 },
                                            { 30, 1 },       { 50, 1 },
                                            { 60000, 1001 }, { 60, 1 } };
      ...
      write_y4m_header(inputparam.g_outfile, headerset->width,
                       headerset->height, fps_num, fps_den,
                       headerset->output_bit_depth);
    }
    return;
  }

  if (inputparam.g_psnr) {
    int ret = cal_psnr(pic->pic_order_count, pic->planes, pic->strides,
                       inputparam.g_recfile, pic->widths[0], pic->lines[0],
                       pic->num_planes, &psnr_y, &psnr_u, &psnr_v,
    ...
    if (ret < 0) {
      ...
      if (inputparam.g_verbose || psnr) {
        show_message(psnr ? CONSOLE_RED : CONSOLE_WHITE,
                     ...
                     g_frmcount, pic->pic_order_count, IMGTYPE[pic->type],
                     pic->qp, psnr_y, psnr_u, psnr_v, pic->pts, pic->dts);
      }
    }
  } else if (inputparam.g_verbose) {
    show_message(CONSOLE_WHITE, "%5d(%d)\t(%c)\t%3d\n", g_frmcount,
                 pic->pic_order_count, IMGTYPE[pic->type], pic->qp);
  }
...
void test_decoder(uint8_t *data_buf, int data_len, int num_frames, char *dst) {
  ...
  param.disable_avx =
      0;  // on some platforms, disable AVX (setting to 1) would be faster
  ...
  for (;;) {
    ...
    // set PTS/DTS, which was only used to check whether they could be passed
    // out rightly
    packet.pts = user_dts;
    packet.dts = -user_dts;
    user_dts++;
  ...
  if (inputparam.g_psnr) {
    if (g_psnrfail == 0 && g_frmcount != 0) {
      show_message(CONSOLE_GREEN, "average PSNR:\t%8.4f, %8.4f, %8.4f\n\n",
                   g_sum_psnr_y / g_frmcount, g_sum_psnr_u / g_frmcount,
                   g_sum_psnr_v / g_frmcount);

      sprintf(dst,
              "  Frames: %d/%d\n  TIME : %.3lfs, %6.2lf fps\n  PSNR : %8.4f, "
              "%8.4f, %8.4f\n",
              g_frmcount, num_frames, (double)((time1 - time0) * f_time_fac),
              (double)(g_frmcount / ((time1 - time0) * f_time_fac)),
              g_sum_psnr_y / g_frmcount, g_sum_psnr_u / g_frmcount,
              g_sum_psnr_v / g_frmcount);
    } else {
      show_message(CONSOLE_RED,
                   "average PSNR:\tNaN, \tNaN, \tNaN\n\n"); /* 'NaN' for 'Not a
                                                               Number' */
    }
  }
...
fail:
  ...
  if (inputparam.s_md5 && strlen(inputparam.s_md5) == 32) {
    ...
    sprintf(MD5str, "%08X%08X%08X%08X", MD5val[0], MD5val[1], MD5val[2],
            MD5val[3]);
