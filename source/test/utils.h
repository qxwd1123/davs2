...
#define LOGE(format, ...) \
  __android_log_print(ANDROID_LOG_ERROR, "davs2", format, ##__VA_ARGS__)
#endif
...
static __inline void show_progress(int frame, int frames) {
  ...
  show_message(CONSOLE_WHITE,
               "\r frames: %4d/%4d,  fps: %4.1f, LeftTime: %8.3f sec\r", frame,
               frames, fps, eta * 0.001);
}

/* ---------------------------------------------------------------------------
 */
static void write_frame_plane(FILE *fp_out, const uint8_t *p_src, int img_w,
                              int img_h, int bytes_per_sample, int i_stride) {
...
/* ---------------------------------------------------------------------------
 */
static void write_y4m_header(FILE *fp, int w, int h, int fps_num, int fps_den,
                             int bit_depth) {
...
/* ---------------------------------------------------------------------------
 */
static void write_frame(davs2_picture_t *pic, FILE *fp, int b_y4m) {
  ...
  /* write y */
  write_frame_plane(fp, pic->planes[0], pic->widths[0], pic->lines[0],
                    bytes_per_sample, pic->strides[0]);

  if (pic->num_planes == 3) {
    /* write u */
    write_frame_plane(fp, pic->planes[1], pic->widths[1], pic->lines[1],
                      bytes_per_sample, pic->strides[1]);

    /* write v */
    write_frame_plane(fp, pic->planes[2], pic->widths[2], pic->lines[2],
                      bytes_per_sample, pic->strides[2]);
  }
}
