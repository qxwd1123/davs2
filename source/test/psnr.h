...
/* ---------------------------------------------------------------------------
 */
static __inline uint64_t cal_ssd_16bit(int width, int height, uint16_t *rec,
                                       int rec_stride, uint16_t *dst,
                                       int dst_stride) {
  uint64_t d = 0;
  int i, j;
...
/* ---------------------------------------------------------------------------
 */
static __inline uint64_t cal_ssd_8bit(int width, int height, uint8_t *rec,
                                      int rec_stride, uint8_t *dst,
                                      int dst_stride) {
  uint64_t d = 0;
  int i, j;
 ...
 * ---------------------------------------------------------------------------
 */
static __inline uint64_t cal_ssd(int width, int height, uint8_t *rec,
                                 int rec_stride, uint8_t *dst, int dst_stride,
                                 int bytes_per_sample) {
  if (bytes_per_sample == 2) {
    return cal_ssd_16bit(width, height, (uint16_t *)rec, rec_stride,
                         (uint16_t *)dst, dst_stride >> 1);
  } else {
...
/* ---------------------------------------------------------------------------
 */
static void find_first_mismatch_point_16bit(int width, int height,
                                            uint16_t *rec, int rec_stride,
...
/* ---------------------------------------------------------------------------
 */
static void find_first_mismatch_point_8bit(int width, int height, uint8_t *rec,
                                           int rec_stride, uint8_t *dst,
 ...
 * ---------------------------------------------------------------------------
 */
static void find_first_mismatch_point(int width, int height, uint8_t *rec,
                                      int rec_stride, uint8_t *dst,
                                      int dst_stride, int bytes_per_sample,
                                      int *x, int *y) {
  if (bytes_per_sample == 2) {
    find_first_mismatch_point_16bit(width, height, (uint16_t *)rec, rec_stride,
                                    (uint16_t *)dst, dst_stride >> 1, x, y);
  } else {
    find_first_mismatch_point_8bit(width, height, rec, rec_stride, dst,
                                   dst_stride, x, y);
  }
}
...
/* ---------------------------------------------------------------------------
 */
static double get_psnr_with_ssd(double f_max, uint64_t diff) {
  if (diff > 0) {
 ...
 * ---------------------------------------------------------------------------
 */
int cal_psnr(int number, uint8_t *dst[3], int strides[3], FILE *f_rec,
             int width, int height, int num_planes, double *psnr_y,
             double *psnr_u, double *psnr_v, int bytes_per_sample,
             int bit_depth) {
  ...
  size_t size_frame = num_planes == 3
                          ? (bytes_per_sample * size_l * 3) >> 1
                          : (bytes_per_sample * size_l);  // solve warning C4018
  ...
  *psnr_y = *psnr_u = *psnr_v = 0.f;
  ...
  diff =
      cal_ssd(width, height, p1, stride_ref, p2, strides[0], bytes_per_sample);
  *psnr_y = get_psnr_with_ssd(f_max_signal * size_l, diff);
  g_sum_psnr_y += *psnr_y;
  if (diff != 0 && b_output_error_position) {
    int x, y;
    find_first_mismatch_point(width, height, p1, stride_ref, p2, strides[0],
                              bytes_per_sample, &x, &y);
  ...
  if (num_planes == 3) {
    ...
    diff = cal_ssd(width, height, p1, stride_ref, p2, strides[1],
                   bytes_per_sample);
    *psnr_u = get_psnr_with_ssd(f_max_signal * size_l, diff << 2);
    g_sum_psnr_u += *psnr_u;
    if (diff != 0 && b_output_error_position) {
      int x, y;
      find_first_mismatch_point(width, height, p1, stride_ref, p2, strides[1],
                                bytes_per_sample, &x, &y);
    ...
    diff = cal_ssd(width, height, p1, stride_ref, p2, strides[2],
                   bytes_per_sample);
    *psnr_v = get_psnr_with_ssd(f_max_signal * size_l, diff << 2);
    g_sum_psnr_v += *psnr_v;
    if (diff != 0 && b_output_error_position) {
      int x, y;
      find_first_mismatch_point(width, height, p1, stride_ref, p2, strides[2],
                                bytes_per_sample, &x, &y);
