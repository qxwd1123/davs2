...
static void display_usage(void) {
  /* 运行参数说明 */
  const char *usage =
      "usage: davs2 -i avs2file -o outputfile [-r recfile] [-t threads] [-v]";
  ...
  show_message(CONSOLE_RED,
               ...
               "--------------+\n");
  show_message(CONSOLE_RED,
               "| --input=test.avs | -i test.avs | input bitstream file path   "
               "              |\n");
  show_message(CONSOLE_RED,
               "| --output=dec.yuv | -o dec.yuv  | output YUV/Y4M file path    "
               "              |\n");
  show_message(CONSOLE_RED,
               "| --psnr=rec.yuv   | -r rec.yuv  | reference reconstruction "
               "YUV file         |\n");
  show_message(CONSOLE_RED,
               "| --threads=N      | -t N        | threads for decoding "
               "(default: 1)         |\n");
  show_message(CONSOLE_RED,
               "| --md5=M          | -m M        | Reference MD5 of decoded "
               "YUV              |\n");
  show_message(CONSOLE_RED,
               "| --verbose        | -v          | Enable decoding status "
               "every frame        |\n");
  show_message(CONSOLE_RED,
...
static int parse_args(davs2_input_param_t *p_param, int argc, char **argv) {
  ...
  while (opt != -1) {
    switch (opt) {
      case 'i': p_param->s_infile = optarg; break;
      case 'o': p_param->s_outfile = optarg; break;
      case 'r': p_param->s_recfile = optarg; break;
      case 'm': p_param->s_md5 = optarg; break;
      case 'v': p_param->g_verbose = 1; break;
      case 't': p_param->g_threads = atoi(optarg); break;
      case 'h': /* fall-through is intentional */
      case '?': display_usage(); return -1;
      case 0: /* long option without a short arg */ break;
      default:
