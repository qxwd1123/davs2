...
/* ---------------------------------------------------------------------------
 * 返回P/F帧的参考帧与当前帧之间的距离 */
static ALWAYS_INLINE int get_distance_index_p(davs2_t *h, int refidx) {
  return h->fdec->dist_refs[refidx];
}

/* ---------------------------------------------------------------------------
 * 返回P/F帧的参考帧与当前帧之间的距离 */
static ALWAYS_INLINE int get_distance_index_p_scale(davs2_t *h, int refidx) {
  return h->fdec->dist_scale_refs[refidx];
}

/* ---------------------------------------------------------------------------
 * 返回B帧的参考帧与当前帧之间的距离 */
static ALWAYS_INLINE int get_distance_index_b(davs2_t *h, int b_fwd) {
  return h->fdec->dist_refs[b_fwd];
}

/* ---------------------------------------------------------------------------
 * 返回B帧的参考帧与当前帧之间的距离 */
static ALWAYS_INLINE int get_distance_index_b_scale(davs2_t *h, int b_fwd) {
  return h->fdec->dist_scale_refs[b_fwd];
}

/* ---------------------------------------------------------------------------
 * 用于场编码中Y分量缩放的偏置 */
static ALWAYS_INLINE int getDeltas(davs2_t *h, int *delt, int *delt2,
                                   int OriPOC, int OriRefPOC, int ScaledPOC,
                                   int ScaledRefPOC) {
  int factor = 2;
...
static ALWAYS_INLINE int16_t scale_mv_default(davs2_t *h, int mv, int dist_dst,
                                              int dist_src) {
  UNUSED_PARAMETER(h);
  mv = davs2_sign3(mv) *
       ((DAVS2_ABS(mv) * dist_dst * dist_src + HALF_MULTI) >> OFFSET);
  return (int16_t)(DAVS2_CLIP3(-32768, 32767, mv));
}

/* ---------------------------------------------------------------------------
 */
static ALWAYS_INLINE int16_t scale_mv_default_y(davs2_t *h, int mvy,
                                                int dist_dst, int dist_src,
                                                int dist_src_mul) {
  if (h->seq_info.b_field_coding) {
    ...
    return (int16_t)(scale_mv_default(h, mvy + delta, dist_dst, dist_src_mul) -
                     delta2);
  } else {
...
static ALWAYS_INLINE int16_t scale_mv_skip(davs2_t *h, int mv, int dist_dst,
                                           int dist_src) {
  UNUSED_PARAMETER(h);
...
static ALWAYS_INLINE int16_t scale_mv_skip_y(davs2_t *h, int mvy, int dist_dst,
                                             int dist_src, int dist_src_mul) {
  if (h->seq_info.b_field_coding) {
    int oriPOC = h->fdec->i_poc;
    int oriRefPOC = oriPOC - dist_src;
    int scaledPOC = h->fdec->i_poc;
    ...
    getDeltas(h, &delta, &delta2, oriPOC, oriRefPOC, scaledPOC, scaledRefPOC);
    return (int16_t)(scale_mv_skip(h, mvy + delta, dist_dst, dist_src_mul) -
                     delta2);
  } else {
...
static ALWAYS_INLINE int16_t scale_mv_biskip(davs2_t *h, int mv, int dist_dst,
                                             int dist_src) {
  UNUSED_PARAMETER(h);
  mv = (int16_t)(davs2_sign3(mv) *
                 ((dist_src * (1 + DAVS2_ABS(mv) * dist_dst) - 1) >> OFFSET));
  return (int16_t)(DAVS2_CLIP3(-32768, 32767, mv));
}

static ALWAYS_INLINE int16_t scale_mv_biskip_y(davs2_t *h, int mvy,
                                               int dist_dst, int dist_src,
                                               int dist_src_mul) {
  if (h->seq_info.b_field_coding) {
    int oriPOC = h->fdec->i_poc;
    ...
    getDeltas(h, &delta, &delta2, oriPOC, oriRefPOC, scaledPOC, scaledRefPOC);
    return (int16_t)(scale_mv_biskip(h, mvy + delta, dist_dst, dist_src_mul) -
                     delta2);
  } else {
...
/* ---------------------------------------------------------------------------
 */
static ALWAYS_INLINE void pmvr_mv_derivation(davs2_t *h, mv_t *mv, mv_t *mvd,
                                             mv_t *mvp) {
  int mvx, mvy;
  ...
  if (h->seq_info.enable_pmvr) {
    ...
    if (DAVS2_ABS(mvd->x - ctr_x) > THRESHOLD_PMVR) {
      mvx = mvp->x + (mvd->x << 1) - ctr_x -
            davs2_sign2(mvd->x - ctr_x) * THRESHOLD_PMVR;
      mvy = mvp->y + (mvd->y << 1) + ctr_y;
    } else if (DAVS2_ABS(mvd->y - ctr_y) > THRESHOLD_PMVR) {
      mvx = mvp->x + (mvd->x << 1) + ctr_x;
      mvy = mvp->y + (mvd->y << 1) - ctr_y -
            davs2_sign2(mvd->y - ctr_y) * THRESHOLD_PMVR;
    } else {
      mvx = mvd->x + mvp->x;
      mvy = mvd->y + mvp->y;
    }
 ...
 */
static ALWAYS_INLINE void cu_get_neighbor_spatial(davs2_t *h, int cur_slice_idx,
                                                  neighbor_inter_t *p_neighbor,
                                                  int x4, int y4) {
  int b_outside_pic =
      y4 < 0 || y4 >= h->i_height_in_spu || x4 < 0 || x4 >= h->i_width_in_spu;
 ...
 */
static ALWAYS_INLINE void cu_get_neighbor_temporal(davs2_t *h,
                                                   neighbor_inter_t *p_neighbor,
                                                   int x4, int y4) {
...
/* ---------------------------------------------------------------------------
 */
static ALWAYS_INLINE int get_pu_type_for_mvp(int bsx, int bsy, int cu_pix_x,
                                             int cu_pix_y) {
  if (bsx < bsy) {
    if (cu_pix_x == 0) {
      return 1;
