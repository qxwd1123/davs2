...
/* ---------------------------------------------------------------------------
 */
static ALWAYS_INLINE cu_t *get_neighbor_cu_in_slice(davs2_t *h, cu_t *p_cur,
                                                    int scu_x, int scu_y,
                                                    int x4x4, int y4x4) {
  const int shift_4x4 = MIN_CU_SIZE_IN_BIT - MIN_PU_SIZE_IN_BIT;

  if (x4x4 < 0 || y4x4 < 0 || x4x4 >= h->i_width_in_spu ||
      y4x4 >= h->i_height_in_spu) {
    return NULL;
  } else if ((scu_x << shift_4x4) <= x4x4 && (scu_y << shift_4x4) <= y4x4) {
    return p_cur;
  } else {
    cu_t *p_neighbor =
        &h->scu_data[(y4x4 >> 1) * h->i_width_in_scu + (x4x4 >> 1)];
 ...
 */
int get_neighbor_cbp_y(davs2_t *h, int x_4x4, int y_4x4, int scu_x, int scu_y,
                       cu_t *p_cu) {
  cu_t *p_neighbor =
      get_neighbor_cu_in_slice(h, p_cu, scu_x, scu_y, x_4x4, y_4x4);
