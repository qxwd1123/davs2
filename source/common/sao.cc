...
/* ---------------------------------------------------------------------------
 */
static void sao_block_eo_0_c(pel_t *p_dst, int i_dst, const pel_t *p_src,
  ...
  for (y = 0; y < i_block_h; y++) {
    ...
    for (x = sx; x < ex; x++) {
      ...
      p_dst[x] =
          (pel_t)DAVS2_CLIP3(0, max_pel_val, p_src[x] + sao_offset[edge_type]);
    }
    p_src += i_src;
    p_dst += i_dst;
  }
...
/* ---------------------------------------------------------------------------
 */
static void sao_block_eo_90_c(pel_t *p_dst, int i_dst, const pel_t *p_src,
                              int i_src, int i_block_w, int i_block_h,
                              int bit_depth, const int *lcu_avail,
                              const int *sao_offset) {
  const int max_pel_val = (1 << bit_depth) - 1;
  ...
  for (x = 0; x < i_block_w; x++) {
    ...
    for (y = sy; y < ey; y++) {
      ...
      top_sign = -down_sign;
      p_dst[y * i_dst + x] = (pel_t)DAVS2_CLIP3(
          0, max_pel_val, p_src[y * i_src + x] + sao_offset[edge_type]);
    }
  }
}
...
/* ---------------------------------------------------------------------------
 */
static void sao_block_eo_135_c(pel_t *p_dst, int i_dst, const pel_t *p_src,
                               int i_src, int i_block_w, int i_block_h,
                               int bit_depth, const int *lcu_avail,
                               const int *sao_offset) {
  ...
  ex_0 =
      lcu_avail[SAO_T] ? (lcu_avail[SAO_R] ? i_block_w : (i_block_w - 1)) : 1;
  for (x = sx_0; x < ex_0; x++) {
...
/* ---------------------------------------------------------------------------
 */
static void sao_block_eo_45_c(pel_t *p_dst, int i_dst, const pel_t *p_src,
                              int i_src, int i_block_w, int i_block_h,
                              int bit_depth, const int *lcu_avail,
                              const int *sao_offset) {
  int8_t SIGN_BUF[MAX_CU_SIZE + 32];  // sign of top line
  ...
  ex_n =
      lcu_avail[SAO_D] ? (lcu_avail[SAO_R] ? i_block_w : (i_block_w - 1)) : 1;
  p_src += i_src;
  p_dst += i_dst;
  for (x = sx_n; x < ex_n; x++) {
    if (x == ex - 1) {
...
/* ---------------------------------------------------------------------------
 */
static void sao_block_bo_c(pel_t *p_dst, int i_dst, const pel_t *p_src,
                           int i_src, int i_block_w, int i_block_h,
...
/* ---------------------------------------------------------------------------
 */
static void sao_read_lcu(davs2_t *h, int lcu_xy, bool_t *slice_sao_on,
                         sao_t *cur_sao_param) {
  ...
  /* neighbor available? */
  merge_top_avail = (scu_y == 0) ? 0
                                 : (h->scu_data[scu_xy].i_slice_nr ==
                                    h->scu_data[scu_xy - w_in_scu].i_slice_nr);
  merge_left_avail = (scu_x == 0) ? 0
                                  : (h->scu_data[scu_xy].i_slice_nr ==
                                     h->scu_data[scu_xy - 1].i_slice_nr);

  if (merge_left_avail || merge_top_avail) {
    merge_mode =
        aec_read_sao_mergeflag(&h->aec, merge_left_avail, merge_top_avail);
  }

  if (merge_mode) {
    if (merge_mode == 2) {
      sao_copy_param(cur_sao_param,
                     &h->lcu_infos[lcu_xy - 1].sao_param);  // copy left
    } else {
      assert(merge_mode == 1);
      sao_copy_param(
          cur_sao_param,
          &h->lcu_infos[lcu_xy - h->i_width_in_lcu].sao_param);  // copy above
    }
  } else {
    int offset[4];
    int stBnd[2];
    ...
    for (i = 0; i < IMG_COMPONENTS; i++) {
      if (!slice_sao_on[i]) {
        ...
        switch (sao_mode) {
          case 0: cur_sao_param->planes[i].modeIdc = SAO_MODE_OFF; break;
          case 1:
        ...
        if (cur_sao_param->planes[i].modeIdc == SAO_MODE_NEW) {
          ...
          if (cur_sao_param->planes[i].typeIdc == SAO_TYPE_BO) {
            memset(cur_sao_param->planes[i].offset, 0,
                   MAX_NUM_SAO_CLASSES * sizeof(int));
            ...
            cur_sao_param->planes[i].offset[SAO_CLASS_EO_FULL_VALLEY] =
                offset[0];
            cur_sao_param->planes[i].offset[SAO_CLASS_EO_HALF_VALLEY] =
                offset[1];
...
/* ---------------------------------------------------------------------------
 */
void sao_read_lcu_param(davs2_t *h, int lcu_xy, bool_t *slice_sao_on,
                        sao_t *sao_param) {
...
/* ---------------------------------------------------------------------------
 */
static void sao_get_neighbor_avail(davs2_t *h, sao_region_t *p_avail,
                                   int i_lcu_x, int i_lcu_y) {
  ...
  if (h->seq_info.cross_loop_filter_flag == FALSE) {
    ...
    if (p_avail->b_top) {
      p_avail->b_top =
          h->scu_data[scu_y * h->i_width_in_scu + scu_x].i_slice_nr ==
          h->scu_data[(scu_y - 1) * h->i_width_in_scu + scu_x].i_slice_nr;
    }
    if (p_avail->b_down) {
...
/* ---------------------------------------------------------------------------
 */
void sao_lcu(davs2_t *h, davs2_frame_t *p_tmp_frm, davs2_frame_t *p_dec_frm,
             int i_lcu_x, int i_lcu_y) {
  ...
  for (comp_idx = 0; comp_idx < IMG_COMPONENTS; comp_idx++) {
    if (h->slice_sao_on[comp_idx] == 0 ||
        lcu_param->planes[comp_idx].modeIdc == SAO_MODE_OFF) {
      continue;
    }
    ...
    if (filter_type == SAO_TYPE_BO) {
      sao_block_bo_c(dst, p_dec_frm->i_stride[comp_idx], src,
                     p_dec_frm->i_stride[comp_idx], region.width[comp_idx],
                     region.height[comp_idx], bit_depth,
                     &lcu_param->planes[comp_idx]);
    } else {
      int avail[8];
      ...
      gf_davs2.sao_filter_eo[filter_type](
          dst, p_dec_frm->i_stride[comp_idx], src,
          p_dec_frm->i_stride[comp_idx], region.width[comp_idx],
          region.height[comp_idx], bit_depth, avail,
          lcu_param->planes[comp_idx].offset);
    }
  }
}
...
/* ---------------------------------------------------------------------------
 */
void sao_lcurow(davs2_t *h, davs2_frame_t *p_tmp_frm, davs2_frame_t *p_dec_frm,
                int i_lcu_y) {
  const int width_in_lcu = h->i_width_in_lcu;
  ...
  for (lcu_x = 0; lcu_x < h->i_width_in_lcu; lcu_x++) {
    ...
    for (comp_idx = 0; comp_idx < IMG_COMPONENTS; comp_idx++) {
      ...
      pel_t *src = p_tmp_frm->planes[comp_idx] + blkoffset;
      if (filter_type == SAO_TYPE_BO) {
        gf_davs2.sao_block_bo(dst, p_dec_frm->i_stride[comp_idx], src,
                              p_dec_frm->i_stride[comp_idx],
                              region.width[comp_idx], region.height[comp_idx],
                              bit_depth, &lcu_param->planes[comp_idx]);
      } else {
        int avail[8];
        avail[0] = region.b_top;
        avail[1] = region.b_down;
