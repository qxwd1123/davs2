...
/* --------------------------------------------------------------------------
 */
static es_unit_t *es_unit_alloc(int buf_size) {
  ...
  if (es_unit == NULL) {
    davs2_log(NULL, DAVS2_LOG_ERROR,
              "failed to malloc memory in es_unit_alloc.\n");
    return NULL;
  }
...
/* --------------------------------------------------------------------------
 */
static void es_unit_free(es_unit_t *es_unit) {
  if (es_unit) {
    davs2_free(es_unit);
  }
}
 ...
 */
static es_unit_t *davs2_pack_es_unit(davs2_mgr_t *mgr, const uint8_t *data,
                                     int len, int64_t pts, int64_t dts) {
#define DAVS2_ISUNIT(x) \
  ((x) == 0xB0 || (x) == 0xB1 || (x) == 0xB7 || (x) == 0xB3 || (x) == 0xB6)
  es_unit_t *es_unit = NULL;
  ...
  if (len > 0) {
    if (es_unit->size < es_unit->len + len) {
      ...
      memcpy(new_es_unit, es_unit,
             sizeof(es_unit_t)); /* copy ES Unit information */
  ...
  if (ret_es_unit != NULL) {
    ret_es_unit->len = bs_dispose_pseudo_code(
        ret_es_unit->data, ret_es_unit->data, ret_es_unit->len);
  }
...
/* ---------------------------------------------------------------------------
 */
static void destroy_all_lists(davs2_mgr_t *mgr) {
  es_unit_t *es_unit = NULL;
  davs2_picture_t *pic = NULL;

  /* idle list */
  for (;;) {
    if ((es_unit = (es_unit_t *)xl_remove_head_ex(&mgr->packets_idle)) ==
        NULL) {
      break;
    }
  ...
  /* recycle list */
  for (;;) {
    if ((pic = (davs2_picture_t *)xl_remove_head_ex(&mgr->pic_recycle)) ==
        NULL) {
      break;
    }
...
/* ---------------------------------------------------------------------------
 */
static int create_all_lists(davs2_mgr_t *mgr) {
  es_unit_t *es_unit = NULL;
  int i;

  if (xl_init(&mgr->packets_idle) != 0 || xl_init(&mgr->pic_recycle) != 0) {
    goto fail;
  }
...
/* ---------------------------------------------------------------------------
 */
static void output_list_recycle_picture(davs2_mgr_t *mgr, davs2_outpic_t *pic) {
  pic->frame = NULL;
  /* picture may be obsolete(for new sequence with different resolution), we
...
/* ---------------------------------------------------------------------------
 */
static int has_new_output_frame(davs2_mgr_t *mgr, davs2_t *h) {
...
/* ---------------------------------------------------------------------------
 */
static davs2_outpic_t *output_list_get_one_output_picture(davs2_mgr_t *mgr) {
  ...
  while (mgr->outpics.pics) {
    ...
    if (frame->i_poc == mgr->outpics.output) {
      ...
      if (frame->i_poc > mgr->outpics.output) {
        ...
        if (mgr->b_flushing &&
            mgr->num_frames_in ==
                mgr->num_frames_out + mgr->outpics.num_output_pic) {
          mgr->outpics.output++;
          continue;
        }
 ...
 */
int decoder_get_output(davs2_mgr_t *mgr, davs2_seq_info_t *headerset,
                       davs2_picture_t *out_frame, int is_flush) {
  davs2_outpic_t *pic = NULL;
  int b_wait_new_frame =
      mgr->num_frames_in + mgr->num_decoders - mgr->num_frames_out >
      8 + mgr->num_aec_thread;
 ...
 * ---------------------------------------------------------------------------
 */
DAVS2_API void davs2_decoder_frame_unref(void *decoder,
...
/* ---------------------------------------------------------------------------
 */
DAVS2_API void *davs2_decoder_open(davs2_param_t *param) {
  const int max_num_thread =
      CTRL_AEC_THREAD ? AVS2_THREAD_MAX : AVS2_THREAD_MAX / 2;
  char buf_cpu[120] = "";
  ...
  if (param->info_level <= DAVS2_LOG_INFO) {
    davs2_log(NULL, DAVS2_LOG_INFO, "davs2: %s.%d, %s", XVERSION_STR, BIT_DEPTH,
              XBUILD_TIME);
  }
  ...
  if (param == NULL) {
    davs2_log(NULL, DAVS2_LOG_ERROR,
              "Invalid input parameters: Null parameters\n");
    return 0;
  }
  ...
  sprintf(mgr->module_log.module_name, "Manager %06llx",
          (long long unsigned int)(mgr));
  ...
  if (mgr->param.threads > max_num_thread) {
    ...
    davs2_log(mgr, DAVS2_LOG_WARNING,
              "Max number of thread reached, forcing to be %d\n",
              max_num_thread);
  }
  ...
  if (mgr->num_total_thread < 1 || mgr->num_decoders < mgr->num_aec_thread ||
      mgr->num_rec_thread < 0 || mgr->num_aec_thread < 1 ||
      mgr->num_aec_thread > mgr->num_total_thread) {
    davs2_log(mgr, DAVS2_LOG_ERROR,
              "Invalid thread number configuration: num_task[%d], "
              "num_threads[%d], num_aec_thread[%d], num_pool[%d]\n",
              mgr->num_decoders, mgr->num_total_thread, mgr->num_aec_thread,
              mgr->num_rec_thread);
    goto fail;
  }
  ...
  davs2_threadpool_init((davs2_threadpool_t **)&mgr->thread_pool,
                        mgr->num_total_thread, NULL, NULL, 0);

  davs2_log(mgr, DAVS2_LOG_INFO,
            "using %d thread(s): %d(frame/AEC)+%d(pool/REC), %d tasks",
            mgr->num_total_thread, mgr->num_aec_thread, mgr->num_rec_thread,
            mgr->num_decoders);
  ...
  if (parse_header(h, &es_unit->bs) == 0) {
    ...
    if (task_get_references(h, es_unit->pts, es_unit->dts) == 0) {
      ...
      davs2_threadpool_run((davs2_threadpool_t *)mgr->thread_pool,
                           decoder_decode_picture_data, h, 0, 0);
    } else {
      davs2_thread_mutex_unlock(&mgr->mutex_aec);
      /* task is free */
...
/* ---------------------------------------------------------------------------
 */
DAVS2_API int davs2_decoder_send_packet(void *decoder, davs2_packet_t *packet) {
  davs2_mgr_t *mgr = (davs2_mgr_t *)decoder;
  es_unit_t *es_unit = NULL;
  ...
  /* check packet length */
  if (packet->len < 4) {
    davs2_log(mgr, DAVS2_LOG_DEBUG,
              "Invalid packet, 4 bytes are needed for one packet (including "
              "start_code). Len = %d",
              packet->len);
    return DAVS2_ERROR; /* error */
  }
  ...
  if (packet->data[0] != 0x00 || packet->data[1] != 0x00 ||
      packet->data[2] != 0x01) {
    davs2_log(mgr, DAVS2_LOG_ERROR,
              "Invalid input Byte-Stream, not start code: %02x%02x%02x",
              packet->data[0], packet->data[1], packet->data[2]);
    return DAVS2_ERROR;
  }

  /* generate one es_unit for current byte-stream buffer */
  es_unit = davs2_pack_es_unit(mgr, packet->data, packet->len, packet->pts,
                               packet->dts);
  if (es_unit == NULL && mgr->es_unit == NULL) {
    davs2_log(mgr, DAVS2_LOG_ERROR,
              "Failed to create an ES_UNIT, input Byte-Stream length %d",
              packet->len);
    return DAVS2_ERROR;
  ...
  if (fp_trace_in) {
    fprintf(fp_trace_in, "\t%8d\t%2d\t%4d\t%3d\t%3d\n", packet->len, ret_type,
            out_frame->pic_order_count, mgr->num_frames_in,
            mgr->num_frames_out);
    fflush(fp_trace_in);
  }
#endif
...
/* ---------------------------------------------------------------------------
 */
DAVS2_API int davs2_decoder_recv_frame(void *decoder,
...
/* ---------------------------------------------------------------------------
 */
DAVS2_API int davs2_decoder_flush(void *decoder, davs2_seq_info_t *headerset,
                                  davs2_picture_t *out_frame) {
  davs2_mgr_t *mgr = (davs2_mgr_t *)decoder;
  int ret;
...
/* ---------------------------------------------------------------------------
 */
DAVS2_API void davs2_decoder_close(void *decoder) {
  davs2_mgr_t *mgr = (davs2_mgr_t *)decoder;
  int i;
