...
/* ---------------------------------------------------------------------------
 */
static const int16_t g_T4[4][4] = { { 32, 32, 32, 32 },
                                    { 42, 17, -17, -42 },
                                    { 32, -32, -32, 32 },
                                    { 17, -42, 42, -17 } };
...
static const int16_t
    g_T32[32][32] = { { 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32,
...
/* ---------------------------------------------------------------------------
 */
static void partialButterflyInverse4_c(const coeff_t *src, coeff_t *dst,
  ...
  for (j = 0; j < line; j++) {
    ...
    dst[0] =
        (coeff_t)DAVS2_CLIP3(min_val, max_val, ((E[0] + O[0] + add) >> shift));
    dst[1] =
        (coeff_t)DAVS2_CLIP3(min_val, max_val, ((E[1] + O[1] + add) >> shift));
    dst[2] =
        (coeff_t)DAVS2_CLIP3(min_val, max_val, ((E[1] - O[1] + add) >> shift));
    dst[3] =
        (coeff_t)DAVS2_CLIP3(min_val, max_val, ((E[0] - O[0] + add) >> shift));
...
/* ---------------------------------------------------------------------------
 */
static void partialButterflyInverse8_c(const coeff_t *src, coeff_t *dst,
                                       int shift, int line, int clip_depth) {
  int E[4], O[4];
  int EE[2], EO[2];
  const int max_val = (1 << (clip_depth - 1)) - 1;
  ...
  for (j = 0; j < line; j++) {
    ...
    for (k = 0; k < 4; k++) {
      dst[k] = (coeff_t)DAVS2_CLIP3(min_val, max_val,
                                    ((E[k] + O[k] + add) >> shift));
      dst[k + 4] = (coeff_t)DAVS2_CLIP3(min_val, max_val,
                                        ((E[3 - k] - O[3 - k] + add) >> shift));
    }
...
/* ---------------------------------------------------------------------------
 */
static void partialButterflyInverse16_c(const coeff_t *src, coeff_t *dst,
                                        int shift, int line, int clip_depth) {
  int E[8], O[8];
  int EE[4], EO[4];
  int EEE[2], EEO[2];
  ...
  for (j = 0; j < line; j++) {
    ...
    for (k = 0; k < 8; k++) {
      dst[k] = (coeff_t)DAVS2_CLIP3(min_val, max_val,
                                    ((E[k] + O[k] + add) >> shift));
      dst[k + 8] = (coeff_t)DAVS2_CLIP3(min_val, max_val,
                                        ((E[7 - k] - O[7 - k] + add) >> shift));
    }
...
/* ---------------------------------------------------------------------------
 */
static void partialButterflyInverse32_c(const coeff_t *src, coeff_t *dst,
                                        int shift, int line, int clip_depth) {
  int E[16], O[16];
  int EE[8], EO[8];
  int EEE[4], EEO[4];
  ...
  for (j = 0; j < line; j++) {
    ...
    for (k = 0; k < 16; k++) {
      dst[k] = (coeff_t)DAVS2_CLIP3(min_val, max_val,
                                    ((E[k] + O[k] + add) >> shift));
      dst[k + 16] = (coeff_t)DAVS2_CLIP3(
          min_val, max_val, ((E[15 - k] - O[15 - k] + add) >> shift));
    }
...
/* ---------------------------------------------------------------------------
 */
static void xTr2nd_4_1d_Inv_Ver(coeff_t *coeff, int i_coeff, int i_shift,
                                const int16_t *tc) {
  ...
  for (i = 0; i < SEC_TR_SIZE; i++) {
    for (j = 0; j < SEC_TR_SIZE; j++) {
      ...
      coeff[i * i_coeff + j] =
          (coeff_t)DAVS2_CLIP3(-32768, 32767, sum >> i_shift);
    }
  }
}
...
/* ---------------------------------------------------------------------------
 */
static void xTr2nd_4_1d_Inv_Hor(coeff_t *coeff, int i_coeff, int i_shift,
  ...
  for (i = 0; i < SEC_TR_SIZE; i++) {
    for (j = 0; j < SEC_TR_SIZE; j++) {
      ...
      coeff[j * i_coeff + i] =
          (coeff_t)DAVS2_CLIP3(min_val, max_val, sum >> i_shift);
    }
  }
}
 ...
 */
static void inv_transform_2nd_c(coeff_t *coeff, int i_coeff, int i_mode,
...
/* ---------------------------------------------------------------------------
 */
static INLINE void inv_transform(davs2_row_rec_t *row_rec, coeff_t *p_coeff,
                                 cu_t *p_cu, int i_coeff, int bsx, int bsy,
                                 int b_secT, int blockidx,
                                 int i_luma_intra_mode) {
  ...
  if (part_idx == PART_4x4) {
    ...
    if (b_secT) {
      gf_davs2.inv_transform_2nd(p_coeff, i_coeff, i_luma_intra_mode,
 ...
 */
static ALWAYS_INLINE coeff_t *get_quanted_coeffs(davs2_row_rec_t *row_rec,
                                                 cu_t *p_cu, int blockidx) {
 ...
 */
void davs2_get_recons(davs2_row_rec_t *row_rec, cu_t *p_cu, int blockidx,
                      cb_t *p_tu, int ctu_x, int ctu_y) {
  int bsx = p_tu->w;
  int bsy = p_tu->h;
  int x_start = p_tu->x;
  int y_start = p_tu->y;
  int b_luma = blockidx < 4;
  int b_wavelet_conducted = (b_luma && p_cu->i_cu_level == B64X64_IN_BIT &&
                             p_cu->i_trans_size != TU_SPLIT_CROSS);
  coeff_t *p_coeff;
  pel_t *p_dst;
  int i_coeff;
  int i_dst;
  ...
  inv_transform(row_rec, p_coeff, p_cu, i_coeff, bsx, bsy,
                h->seq_info.enable_2nd_transform, blockidx,
                p_cu->intra_pred_modes[blockidx]);
  ...
  // normalize
  gf_davs2.add_ps[PART_INDEX(bsx, bsy)](p_dst, i_dst, p_dst, p_coeff, i_dst,
                                        i_coeff);
}
...
void davs2_dct_init(uint32_t cpuid, ao_funcs_t *fh) {
  ...
  if (cpuid & DAVS2_CPU_AVX2) {
    ...
    fh->idct[PART_32x32][DCT_DEAULT] =
        idct_32x32_avx2;  // @luofl i7-6700k 速度比sse128快一倍
