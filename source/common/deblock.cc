...
enum edge_type_e {
  ...
  EDGE_TYPE_ONLY_LUMA =
      1, /* TU boundary in CU (chroma block does not have such boundaries) */
  EDGE_TYPE_BOTH = 2 /* CU boundary and PU boundary */
};

/* ---------------------------------------------------------------------------
 */
static void lf_set_edge_filter_param(davs2_t *h, int i_level, int scu_x,
                                     int scu_y, int dir, int edge_type) {
  ...
  if (dir == EDGE_VER) {
    ...
    for (i = 0; i < scu_num; i++) {
      if (h->p_deblock_flag[EDGE_VER][(scu_y + i) * w_in_scu + scu_x] !=
          EDGE_TYPE_NOFILTER) {
        break;
      }
      h->p_deblock_flag[EDGE_VER][(scu_y + i) * w_in_scu + scu_x] =
          (uint8_t)edge_type;
    }
  } else {
     ...
     * check edge condition, can not filter beyond frame/slice boundaries */
    if (!h->seq_info.cross_loop_filter_flag &&
        h->scu_data[scu_xy].i_slice_nr !=
            h->scu_data[scu_xy - h->i_width_in_scu].i_slice_nr) {
      return;
    }
    ...
    for (i = 0; i < scu_num; i++) {
      if (h->p_deblock_flag[EDGE_HOR][scu_y * w_in_scu + scu_x + i] !=
          EDGE_TYPE_NOFILTER) {
        break;
      }
      h->p_deblock_flag[EDGE_HOR][scu_y * w_in_scu + scu_x + i] =
          (uint8_t)edge_type;
    }
  }
}
/* ---------------------------------------------------------------------------
 */
static void lf_lcu_set_edge_filter(davs2_t *h, int i_level, int scu_x,
                                   int scu_y) {
  const int w_in_scu = h->i_width_in_scu;
  ...
  if (p_scu_data->i_cu_level < i_level) {
    ...
    lf_set_edge_filter_param(h, i_level, scu_x, scu_y, EDGE_VER,
                             EDGE_TYPE_BOTH);  // left edge
    lf_set_edge_filter_param(h, i_level, scu_x, scu_y, EDGE_HOR,
                             EDGE_TYPE_BOTH);  // top  edge
    ...
    if (p_scu_data->i_cu_level > B8X8_IN_BIT) {
      ...
      switch (p_scu_data->i_cu_type) {
        case PRED_2NxN:
          lf_set_edge_filter_param(h, i_level, scu_x, scu_y + (1 << i),
                                   EDGE_HOR, EDGE_TYPE_BOTH);
          break;
        case PRED_Nx2N:
          lf_set_edge_filter_param(h, i_level, scu_x + (1 << i), scu_y,
                                   EDGE_VER, EDGE_TYPE_BOTH);
          break;
        case PRED_I_NxN:
          lf_set_edge_filter_param(h, i_level, scu_x + (1 << i), scu_y,
                                   EDGE_VER, EDGE_TYPE_BOTH);
        ...
        case PRED_I_2Nxn:
          if (i > 0) {
            lf_set_edge_filter_param(h, i_level, scu_x, scu_y + (1 << (i - 1)),
                                     EDGE_HOR, EDGE_TYPE_ONLY_LUMA);
            lf_set_edge_filter_param(h, i_level, scu_x,
                                     scu_y + (1 << (i - 1)) * 2, EDGE_HOR,
                                     EDGE_TYPE_ONLY_LUMA);
            lf_set_edge_filter_param(h, i_level, scu_x,
                                     scu_y + (1 << (i - 1)) * 3, EDGE_HOR,
                                     EDGE_TYPE_ONLY_LUMA);
          } else {
            lf_set_edge_filter_param(h, i_level, scu_x, scu_y + (1 << (i)),
                                     EDGE_HOR, EDGE_TYPE_ONLY_LUMA);
        ...
        case PRED_I_nx2N:
          if (i > 0) {
            lf_set_edge_filter_param(h, i_level, scu_x + (1 << (i - 1)), scu_y,
                                     EDGE_VER, EDGE_TYPE_ONLY_LUMA);
            lf_set_edge_filter_param(h, i_level, scu_x + (1 << (i - 1)) * 2,
                                     scu_y, EDGE_VER, EDGE_TYPE_ONLY_LUMA);
            lf_set_edge_filter_param(h, i_level, scu_x + (1 << (i - 1)) * 3,
                                     scu_y, EDGE_VER, EDGE_TYPE_ONLY_LUMA);
          } else {
            lf_set_edge_filter_param(h, i_level, scu_x + (1 << (i)), scu_y,
                                     EDGE_VER, EDGE_TYPE_ONLY_LUMA);
        ...
        case PRED_nRx2N:
          if (i > 0) {
            lf_set_edge_filter_param(h, i_level, scu_x + (1 << (i - 1)) * 3,
                                     scu_y, EDGE_VER, EDGE_TYPE_BOTH);
          }
          break;
        default:
          // for other modes: direct/skip, 2Nx2N inter, 2Nx2N intra, no need to
          // set
          break;
      }

      /* set transform block boundary */
      if (p_scu_data->i_cu_type != PRED_I_NxN &&
          p_scu_data->i_trans_size != TU_SPLIT_NON && p_scu_data->i_cbp != 0) {
        ...
        } else if (h->seq_info.enable_nsqt &&
                   IS_VER_PU_PART(p_scu_data->i_cu_type)) {
          if (p_scu_data->i_cu_level == B16X16_IN_BIT) {
            lf_set_edge_filter_param(h, i_level, scu_x + (1 << (i)), scu_y,
                                     EDGE_VER, EDGE_TYPE_ONLY_LUMA);
          } else {
            ...
            lf_set_edge_filter_param(h, i_level, scu_x + (1 << (i)), scu_y,
                                     EDGE_VER, EDGE_TYPE_ONLY_LUMA);
            lf_set_edge_filter_param(h, i_level,
                                     scu_x + (1 << (i)) + (1 << (i - 1)), scu_y,
                                     EDGE_VER, EDGE_TYPE_ONLY_LUMA);
          }
        } else {
          lf_set_edge_filter_param(h, i_level, scu_x + (1 << i), scu_y,
 ...
 */
static uint8_t lf_skip_filter(davs2_t *h, cu_t *scuP, cu_t *scuQ, int dir,
  ...
  if (h->i_frame_type == AVS2_P_SLICE || h->i_frame_type == AVS2_F_SLICE) {
    ...
    if ((scuP->i_cbp == 0) && (scuQ->i_cbp == 0) &&
        (DAVS2_ABS(mv_1.x - mv_2.x) < 4) && (DAVS2_ABS(mv_1.y - mv_2.y) < 4) &&
...
/* ---------------------------------------------------------------------------
 */
static void lf_edge_core(pel_t *src, int b_chroma, int ptr_inc, int inc1,
  ...
  int fs;  // fs stands for filtering strength.  The larger fs is, the stronger
           // filter is applied.
  int FlatnessL, FlatnessR;  // FlatnessL and FlatnessR describe how flat the
                             // curve is of one coding unit
  int flag;
  int pel;
  ...
  for (pel = 0; pel < MIN_CU_SIZE; pel++) {
    ...
    if (flag && (abs_delta < alpha) && (abs_delta > 1)) {
      ...
      switch (FlatnessL + FlatnessR) {
        case 6:
          fs = 3 + ((R1 == R0) &&
                    (L0 == L1));  // ((R1 == R0) && (L0 == L1)) ? 4 : 3;
          break;
        case 5:
          fs = 2 + ((R1 == R0) &&
                    (L0 == L1));  // ((R1 == R0) && (L0 == L1)) ? 3 : 2;
          break;
        case 4:
          fs = 1 + (FlatnessL == 2);  // (FlatnessL == 2) ? 2 : 1;
          break;
        case 3: fs = (DAVS2_ABS(L1 - R1) < beta); break;
        default: fs = 0;
      }
      ...
      switch (fs) {
        case 4:
          src[-inc1] = (pel_t)((L0 + ((L0 + L2) << 3) + L2 + (R0 << 3) +
                                (R2 << 2) + (R2 << 1) + 16) >>
                               5);  // L0
          src[-inc2] = (pel_t)(((L0 << 3) - L0 + (L2 << 2) + (L2 << 1) + R0 +
                                (R0 << 1) + 8) >>
                               4);  // L1
          src[-inc3] =
              (pel_t)(((L0 << 2) + L2 + (L2 << 1) + R0 + 4) >> 3);  // L2
          src[0] = (pel_t)((R0 + ((R0 + R2) << 3) + R2 + (L0 << 3) + (L2 << 2) +
                            (L2 << 1) + 16) >>
                           5);  // R0
          src[inc1] = (pel_t)(((R0 << 3) - R0 + (R2 << 2) + (R2 << 1) + L0 +
                               (L0 << 1) + 8) >>
                              4);  // R1
          src[inc2] =
              (pel_t)(((R0 << 2) + R2 + (R2 << 1) + L0 + 4) >> 3);  // R2
          break;
        case 3:
          src[-inc1] = (pel_t)((L2 + (L1 << 2) + (L0 << 2) + (L0 << 1) +
                                (R0 << 2) + R1 + 8) >>
                               4);  // L0
          src[0] = (pel_t)((L1 + (L0 << 2) + (R0 << 2) + (R0 << 1) + (R1 << 2) +
                            R2 + 8) >>
                           4);  // R0
          ...
          break;
        case 2:
          src[-inc1] = (pel_t)(((L1 << 1) + L1 + (L0 << 3) + (L0 << 1) +
                                (R0 << 1) + R0 + 8) >>
                               4);
          src[0] = (pel_t)(((L0 << 1) + L0 + (R0 << 3) + (R0 << 1) + (R1 << 1) +
                            R1 + 8) >>
                           4);
          break;
        case 1:
...
/* ---------------------------------------------------------------------------
 */
static void deblock_edge_hor(pel_t *src, int stride, int alpha, int beta,
                             uint8_t *flt_flag) {
...
/* ---------------------------------------------------------------------------
 */
static void deblock_edge_ver(pel_t *src, int stride, int alpha, int beta,
                             uint8_t *flt_flag) {
...
static void deblock_edge_ver_c(pel_t *src_u, pel_t *src_v, int stride,
                               int *alpha, int *beta, uint8_t *flt_flag)
#else
static void deblock_edge_ver_c(pel_t *src_u, pel_t *src_v, int stride,
                               int alpha, int beta, uint8_t *flt_flag)
 ...
 */
#if HDR_CHROMA_DELTA_QP
static void deblock_edge_hor_c(pel_t *src_u, pel_t *src_v, int stride,
                               int *alpha, int *beta, uint8_t *flt_flag)
#else
static void deblock_edge_hor_c(pel_t *src_u, pel_t *src_v, int stride,
                               int alpha, int beta, uint8_t *flt_flag)
 ...
 */
static void lf_scu_deblock(davs2_t *h, pel_t *p_dec[3], int stride,
                           int stride_c, int scu_x, int scu_y, int dir) {
  ...
  if (edge_condition != EDGE_TYPE_NOFILTER) {
    ...
    b_filter_flag[0] =
        lf_skip_filter(h, scuP, scuQ, dir, (scu_x << 1), (scu_y << 1));
    b_filter_flag[1] = lf_skip_filter(h, scuP, scuQ, dir, (scu_x << 1) + dir,
                                      (scu_y << 1) + !dir);
    ...
    /* deblock luma edge */
    {
      pel_t *src_y = p_dec[0] + (scu_y << MIN_CU_SIZE_IN_BIT) * stride +
                     (scu_x << MIN_CU_SIZE_IN_BIT);
      int alpha, beta;
      QP =
          ((scuP->i_qp + scuQ->i_qp + 1) >> 1);  // average QP of the two blocks

      /* coded as 10/12 bit, QP is added by (8 * (h->param.sample_bit_depth -
       * 8)) in config file */
      alpha = ALPHA_TABLE[DAVS2_CLIP3(0, max_qp_deblock,
                                      QP - (shift << 3) + h->i_alpha_offset)]
              << shift;
      beta = BETA_TABLE[DAVS2_CLIP3(0, max_qp_deblock,
                                    QP - (shift << 3) + h->i_beta_offset)]
             << shift;
    ...
    if (edge_condition == EDGE_TYPE_BOTH && h->i_chroma_format != CHROMA_400)
      if (((scu_y & 1) == 0 && dir) || (((scu_x & 1) == 0) && (!dir))) {
        int uv_offset = (scu_y << (MIN_CU_SIZE_IN_BIT - 1)) * stride_c +
                        (scu_x << (MIN_CU_SIZE_IN_BIT - 1));
        ...
        alpha[0] =
            ALPHA_TABLE[DAVS2_CLIP3(0, max_qp_deblock, QP + h->i_alpha_offset)]
            << shift;
        beta[0] =
            BETA_TABLE[DAVS2_CLIP3(0, max_qp_deblock, QP + h->i_beta_offset)]
            << shift;
        ...
        gf_davs2.deblock_chroma[dir](src_u, src_v, stride_c, alpha, beta,
                                     b_filter_flag);
#else
        int alpha, beta;
        ...
        alpha =
            ALPHA_TABLE[DAVS2_CLIP3(0, max_qp_deblock, QP + h->i_alpha_offset)]
            << shift;
        beta = BETA_TABLE[DAVS2_CLIP3(0, max_qp_deblock, QP + h->i_beta_offset)]
               << shift;
 ...
 */
void davs2_lcu_deblock(davs2_t *h, davs2_frame_t *frm, int i_lcu_x,
                       int i_lcu_y) {
  ...
  for (j = 0; j < num_of_scu_ver; j++) {
    for (i = 0; i < num_of_scu_hor; i++) {
      lf_scu_deblock(h, frm->planes, i_stride, i_stride_c, scu_x + i, scu_y + j,
                     EDGE_VER);
    }
  }
...
#if !HIGH_BIT_DEPTH
    // fh->deblock_luma[0] = deblock_edge_ver_avx2;  // @luofl i7-6700K
    // 此函数慢于 sse128 fh->deblock_luma[1] = deblock_edge_hor_avx2;
