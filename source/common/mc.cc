...
ALIGN16(static const int8_t INTPL_FILTERS_C[8][4]) = {
  ...
  { -4, 62, 6, 0 },   { -6, 56, 15, -1 }, { -5, 47, 25, -3 },
  { -4, 36, 36, -4 }, { -3, 25, 47, -5 }, { -1, 15, 56, -6 },
  { 0, 6, 62, -4 }
};
...
#define FLT_8TAP_HOR(src, i, coef)                       \
  ((src)[i - 3] * (coef)[0] + (src)[i - 2] * (coef)[1] + \
   (src)[i - 1] * (coef)[2] + (src)[i] * (coef)[3] +     \
   (src)[i + 1] * (coef)[4] + (src)[i + 2] * (coef)[5] + \
   (src)[i + 3] * (coef)[6] + (src)[i + 4] * (coef)[7])

/* ---------------------------------------------------------------------------
 * for luma interpolating (vertical) */
#define FLT_8TAP_VER(src, i, i_src, coef)                                \
  ((src)[i - 3 * i_src] * (coef)[0] + (src)[i - 2 * i_src] * (coef)[1] + \
   (src)[i - i_src] * (coef)[2] + (src)[i] * (coef)[3] +                 \
   (src)[i + i_src] * (coef)[4] + (src)[i + 2 * i_src] * (coef)[5] +     \
   (src)[i + 3 * i_src] * (coef)[6] + (src)[i + 4 * i_src] * (coef)[7])

/* ---------------------------------------------------------------------------
 * for chroma interpolating (horizontal) */
#define FLT_4TAP_HOR(src, i, coef)                   \
  ((src)[i - 1] * (coef)[0] + (src)[i] * (coef)[1] + \
   (src)[i + 1] * (coef)[2] + (src)[i + 2] * (coef)[3])

/* ---------------------------------------------------------------------------
 * for chroma interpolating (vertical) */
#define FLT_4TAP_VER(src, i, i_src, coef)                \
  ((src)[i - i_src] * (coef)[0] + (src)[i] * (coef)[1] + \
   (src)[i + i_src] * (coef)[2] + (src)[i + 2 * i_src] * (coef)[3])
...
/* ---------------------------------------------------------------------------
 */
static void mc_block_copy_c(pel_t *dst, intptr_t i_dst, pel_t *src,
...
/* ---------------------------------------------------------------------------
 */
static void mc_block_copy_sc_c(coeff_t *dst, intptr_t i_dst, int16_t *src,
                               intptr_t i_src, int w, int h) {
  int i;
...
/* ---------------------------------------------------------------------------
 */
static void intpl_chroma_block_hor_c(pel_t *dst, int i_dst, pel_t *src,
...
/* ---------------------------------------------------------------------------
 */
static void intpl_chroma_block_ver_c(pel_t *dst, int i_dst, pel_t *src,
                                     int i_src, int width, int height,
                                     int8_t const *coeff) {
  int x, y, v;
...
/* ---------------------------------------------------------------------------
 */
static void intpl_chroma_block_ext_c(pel_t *dst, int i_dst, pel_t *src,
                                     int i_src, int width, int height,
                                     const int8_t *coeff_h,
...
/* ---------------------------------------------------------------------------
 */
static void intpl_luma_block_hor_c(pel_t *dst, int i_dst, pel_t *src, int i_src,
...
/* ---------------------------------------------------------------------------
 */
static void intpl_luma_block_ver_c(pel_t *dst, int i_dst, pel_t *src, int i_src,
                                   int width, int height, int8_t const *coeff) {
  int x, y, v;
...
/* ---------------------------------------------------------------------------
 */
static void intpl_luma_block_ext_c(pel_t *dst, int i_dst, pel_t *src, int i_src,
...
#define INTERP_HOR_C(width, height)                                         \
  static void interp_horiz_pp_##width##x##height##_c(                       \
      const pel_t *src, intptr_t srcStride, pel_t *dst, intptr_t dstStride, \
      int coeffIdx) {                                                       \
    const int N = 8; /* 8-tap Luma interpolation */                         \
    const int8_t *coeff =                                                   \
        (N == 4) ? INTPL_FILTERS_C[coeffIdx] : INTPL_FILTERS[coeffIdx];     \
...
#define INTERP_PS_HOR_C(width, height)                                        \
  static void interp_horiz_ps_##width##x##height##_c(                         \
      const pel_t *src, intptr_t srcStride, int16_t *dst, intptr_t dstStride, \
      int coeffIdx) {                                                         \
    const int N = 8; /* 8-tap Luma interpolation */                           \
    const int8_t *coeff =                                                     \
        (N == 4) ? INTPL_FILTERS_C[coeffIdx] : INTPL_FILTERS[coeffIdx];       \
...
#define INTERP_VER_C(width, height)                                         \
  static void interp_vert_pp_##width##x##height##_c(                        \
      const pel_t *src, intptr_t srcStride, pel_t *dst, intptr_t dstStride, \
      int coeffIdx) {                                                       \
    const int N = 8; /* 8-tap Luma interpolation */                         \
    const int8_t *c =                                                       \
...
#define INTERP_SP_VER_C(w, h)                                                 \
  static void filterVertical_sp_##w##x##h##_c(                                \
      const int16_t *src, intptr_t srcStride, pel_t *dst, intptr_t dstStride, \
      int coeffIdx) {                                                         \
    const int N = 8; /* 8-tap Luma interpolation */                           \
    ...
    const int8_t *c =                                                         \
        (N == 8 ? INTPL_FILTERS_C[coeffIdx] : INTPL_FILTERS[coeffIdx]);       \
...
#define INTERP_EXT_C(width, height)                                          \
  static void interp_hv_pp_##width##x##height##_c(                           \
      const pel_t *src, intptr_t srcStride, pel_t *dst, intptr_t dstStride,  \
      int idxX, int idxY) {                                                  \
    int16_t immedVals[(64 + 8) * (64 + 8)];                                  \
    interp_horiz_ps_##width##x##height##_c(src, srcStride, immedVals, width, \
                                           idxX);                            \
    filterVertical_sp_##width##x##height##_c(immedVals + 3 * width, width,   \
                                             dst, dstStride, idxY);          \
  }
 ...
 *      C           D
 */
void mc_luma(davs2_t *h, pel_t *dst, int i_dst, int posx, int posy, int width,
             int height, pel_t *p_fref, int i_fref) {
  const int dx = posx & 3;
  const int dy = posy & 3;
...
#if USE_NEW_INTPL
    gf_davs2.block_intpl_luma_ver[PART_INDEX(width, height)](p_fref, i_fref,
                                                             dst, i_dst, dy);
#else
    gf_davs2.intpl_luma_ver[mc_part_index][dy - 1](
        dst, i_dst, p_fref, i_fref, width, height, INTPL_FILTERS[dy]);
#endif
  } else if (dy == 0) {
#if USE_NEW_INTPL
    gf_davs2.block_intpl_luma_hor[PART_INDEX(width, height)](p_fref, i_fref,
                                                             dst, i_dst, dx);
#else
    gf_davs2.intpl_luma_hor[mc_part_index][dx - 1](
        dst, i_dst, p_fref, i_fref, width, height, INTPL_FILTERS[dx]);
#endif
  } else {
    gf_davs2.intpl_luma_ext[mc_part_index](dst, i_dst, p_fref, i_fref, width,
                                           height, INTPL_FILTERS[dx],
                                           INTPL_FILTERS[dy]);
  }
}
...
/* ---------------------------------------------------------------------------
 */
void mc_chroma(davs2_t *h, pel_t *dst, int i_dst, int posx, int posy, int width,
               int height, pel_t *p_fref, int i_fref) {
  const int dx = posx & 7;
  const int dy = posy & 7;
  const int mc_part_index = MC_PART_INDEX(width, height);
  ...
  if (dx == 0 && dy == 0) {
    ...
    gf_davs2.intpl_chroma_ver[mc_part_index](dst, i_dst, p_fref, i_fref, width,
                                             height, INTPL_FILTERS_C[dy]);
  } else if (dy == 0) {
    gf_davs2.intpl_chroma_hor[mc_part_index](dst, i_dst, p_fref, i_fref, width,
                                             height, INTPL_FILTERS_C[dx]);
  } else {
    gf_davs2.intpl_chroma_ext[mc_part_index](dst, i_dst, p_fref, i_fref, width,
                                             height, INTPL_FILTERS_C[dx],
                                             INTPL_FILTERS_C[dy]);
  }
}
...
/* ---------------------------------------------------------------------------
 */
static void davs2_pixel_average_c(pel_t *dst, int i_dst, const pel_t *src0,
