 ...
 */
static INLINE int read_bits(uint8_t *p_buf, int i_buf, int i_bit_pos,
                            int *p_info, int i_bits) {
 ...
 */
static INLINE int get_vlc_symbol(uint8_t *p_buf, int i_bit_pos, int *info,
                                 int i_buf) {
  int byte_offset = i_bit_pos >> 3;      // byte from start of buffer
  int bit_offset = 7 - (i_bit_pos & 7);  // bit from start of byte
  int bit_counter = 1;
 ...
 */
static INLINE int vlc_u_v(davs2_bs_t *bs, int i_bits
#if AVS2_TRACE
                          ,
                          char *tracestring
#endif
  ...
  if (read_bits(bs->p_stream, bs->i_stream, bs->i_bit_pos, &ret_val, i_bits) ==
      0) {
 ...
 * RETURN: the value of the coded syntax element, or -1 on error
 */
static INLINE int vlc_ue_v(davs2_bs_t *bs
#if AVS2_TRACE
                           ,
                           char *tracestring
#endif
 ...
 * RETURN: the value of the coded syntax element, or -1 on error
 */
static INLINE int vlc_se_v(davs2_bs_t *bs
#if AVS2_TRACE
                           ,
                           char *tracestring
#endif
