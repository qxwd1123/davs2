...
int found_slice_header(davs2_bs_t *bs) {
  ...
  for (; num_bytes; num_bytes--) {
    ...
    if ((code & 0x00FFFFFF) == 0x00010000 &&
        ((code >> 24) <= SC_SLICE_CODE_MAX)) {
      return 1;
    }
    bs->i_bit_pos += 8;
  }
...
int bs_get_start_code(davs2_bs_t *bs) {
  ...
  while (i_left_bytes >= 4 &&
         (*(uint32_t *)p_data & 0x00FFFFFF) != 0x00010000) {
    p_data++;
    i_left_bytes--;
    i_used_bytes++;
  }
...
int bs_dispose_pseudo_code(uint8_t *dst, uint8_t *src, int i_src) {
  ...
  while (i_pos < i_src) {
    ...
    switch (curr_byte) {
      ...
      default:
        if (b_found_start_code) {
          if (curr_byte == SC_SEQUENCE_HEADER || curr_byte == SC_USER_DATA ||
              curr_byte == SC_EXTENSION) {
            b_dispose = 0;
          } else {
            b_dispose = 1;
          }
    ...
    if (curr_bit_count == 8) {
      if (last_bit_count == 0) {
        ...
        dst[i_dst++] =
            ((last_byte & BITMASK[last_bit_count]) |
             ((curr_byte & BITMASK[8 - last_bit_count]) >> last_bit_count));
        last_byte =
            (curr_byte << (8 - last_bit_count)) & BITMASK[last_bit_count];
      }
    } else {
      if (last_bit_count == 0) {
        last_byte = curr_byte;
        ...
        dst[i_dst++] =
            ((last_byte & BITMASK[last_bit_count]) |
             ((curr_byte & BITMASK[8 - last_bit_count]) >> last_bit_count));
        last_byte =
            (curr_byte << (8 - last_bit_count)) & BITMASK[last_bit_count - 2];
...
int32_t find_pic_start_code(uint8_t prevbyte3, uint8_t prevbyte2,
                            uint8_t prevbyte1, const uint8_t *data,
                            int32_t len) {
#define ISPIC(x) \
  ((x) == 0xB0 || (x) == 0xB1 || (x) == 0xB3 || (x) == 0xB6 || (x) == 0xB7)
  ...
  if (/*..*/ len >= 1 && (prevbyte3 == 0) && (prevbyte2 == 0) &&
      (prevbyte1 == 1)) {
    if (ISPIC(data[0])) {
      return -3;  // found start code (position: -3)
    }
  } else if (len >= 2 && (prevbyte2 == 0) && (prevbyte1 == 0) &&
             (data[0] == 1)) {
    if (ISPIC(data[1])) {
  ...
  while (((p = (uint8_t *)find_start_code(data, len)) != NULL) &&
         !ISPIC(p[3])) {
