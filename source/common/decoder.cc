...
static void init_frame(davs2_t *h) {
  ...
  memset(h->p_ipredmode - h->i_ipredmode - 16, DC_PRED,
         h->i_ipredmode * (h->i_height_in_spu + 1) * sizeof(int8_t));
  ...
  if (h->b_alf) {
    int alf_enable = h->pic_alf_on[IMG_Y] != 0 || h->pic_alf_on[IMG_U] != 0 ||
                     h->pic_alf_on[IMG_V] != 0;
    if (alf_enable) {
  ...
  gf_davs2.fast_memzero(
      h->p_deblock_flag[0],
      h->i_width_in_scu * h->i_height_in_scu * 2 * sizeof(uint8_t));

  /* 7, clear LCU info buffer */
#if CTRL_AEC_THREAD
  gf_davs2.fast_memzero(h->lcu_infos, sizeof(lcu_info_t) * h->i_width_in_lcu *
                                          h->i_height_in_lcu);
#endif
}
 ...
 * cache CTU border
 */
static INLINE void davs2_cache_lcu_border(pel_t *p_dst, const pel_t *p_top,
 ...
 */
static INLINE void davs2_cache_lcu_border_uv(
    pel_t *p_dst_u, const pel_t *p_top_u, const pel_t *p_left_u, pel_t *p_dst_v,
    const pel_t *p_top_v, const pel_t *p_left_v, int i_left, int lcu_width,
    int lcu_height) {
  int i;
  /* top, top-right */
 ...
 * 等待一行LCU熵解码完指定数量的LCU
 */
static ALWAYS_INLINE void wait_lcu_row_parsed(davs2_t *h, davs2_frame_t *frm,
                                              int lcu_xy) {
  UNUSED_PARAMETER(h);
 ...
 */
static ALWAYS_INLINE void wait_lcu_row_reconed(davs2_t *h, davs2_frame_t *frm,
                                               int wait_lcu_y,
                                               int wait_lcu_coded) {
  UNUSED_PARAMETER(h);
  ...
  if (frm->num_decoded_lcu_in_row[wait_lcu_y] < wait_lcu_coded) {
    ...
    while (frm->num_decoded_lcu_in_row[wait_lcu_y] < wait_lcu_coded) {
      davs2_thread_cond_wait(&frm->conds_lcu_row[wait_lcu_y],
                             &frm->mutex_recon);
    }
...
/* ---------------------------------------------------------------------------
 */
static void task_send_picture_to_output_list(davs2_t *h, davs2_outpic_t *pic) {
  ...
  if (curr != NULL && curr->frame->i_poc == pic->frame->i_poc) {
    davs2_log(h, DAVS2_LOG_WARNING, "detected duplicate POC %d",
              curr->frame->i_poc);
  }
  ...
  DAVS2_ASSERT(h->task_info.task_status == TASK_BUSY, "Invalid task status %d",
...
/* ---------------------------------------------------------------------------
 */
static void task_output_decoding_frame(davs2_t *h) {
  davs2_mgr_t *mgr = h->task_info.taskmgr;
  ...
  if (frame->i_type == AVS2_GB_SLICE) {
    pic->frame = h->f_background_ref;  ///!!! FIXME: actually NOT working (we do
                                       ///not support S frames now).
  } else {
    pic->frame = frame;
  }
...
/* ---------------------------------------------------------------------------
 */
static ALWAYS_INLINE int check_slice_header(davs2_t *h, davs2_bs_t *bs,
                                            int lcu_y) {
  aec_t *p_aec = &h->aec;
  ...
  if (h->b_slice_checked && found_slice_header(bs)) {
    ...
    aec_new_slice(h);
    aec_start_decoding(p_aec, bs->p_stream, ((bs->i_bit_pos + 7) / 8),
                       bs->i_stream);
    AEC_RETURN_ON_ERROR(-1);

    /* 当前Slice的上一行的预测模式清空 */
    lcu_y <<= (h->i_lcu_level - MIN_PU_SIZE_IN_BIT);
    memset(h->p_ipredmode + (lcu_y - 1) * h->i_ipredmode - 16, DC_PRED,
           h->i_ipredmode * sizeof(int8_t));
  }
...
/* ---------------------------------------------------------------------------
 */
static ALWAYS_INLINE void rowrec_store_lcu_recon_samples(
...
static int decode_one_lcu_row(davs2_t *h, davs2_bs_t *bs, int i_lcu_y) {
   ...
   */
  for (i_lcu_x = 0; i_lcu_x < width_in_lcu && h->decoding_error == 0;
       i_lcu_x++, lcu_xy++) {
    ...
    if (h->b_sao) {
      sao_read_lcu_param(h, lcu_xy, h->slice_sao_on,
                         &h->lcu.lcu_aec->sao_param);
    }

    if (h->b_alf) {
      for (i = 0; i < IMG_COMPONENTS; i++) {
        if (h->pic_alf_on[i]) {
          h->lcu.lcu_aec->enable_alf[i] =
              (uint8_t)aec_read_alf_lcu_ctrl(&h->aec);
        } else {
    ...
    if (i_lcu_x == 0) {
      memcpy(row_rec.ctu_border[0].rec_top + 1, h->intra_border[0],
             row_rec.ctu.i_ctu_w * 2 * sizeof(pel_t));
      memcpy(row_rec.ctu_border[1].rec_top + 1, h->intra_border[1],
             row_rec.ctu.i_ctu_w * sizeof(pel_t));
      memcpy(row_rec.ctu_border[2].rec_top + 1, h->intra_border[2],
             row_rec.ctu.i_ctu_w * sizeof(pel_t));
    }

    decode_lcu_recon(h, &row_rec, h->i_lcu_level, h->lcu.i_pix_x,
                     h->lcu.i_pix_y);

    rowrec_store_lcu_recon_samples(&row_rec);
    /* cache top and left samples for intra prediction of next CTU */
    davs2_cache_lcu_border(
        row_rec.ctu_border[0].rec_top,
        h->intra_border[0] + row_rec.ctu.i_pix_x + row_rec.ctu.i_ctu_w - 1,
        row_rec.ctu.p_frec[0] + row_rec.ctu.i_ctu_w - 1, row_rec.ctu.i_frec[0],
        row_rec.ctu.i_ctu_w, row_rec.ctu.i_ctu_h);
    davs2_cache_lcu_border_uv(
        row_rec.ctu_border[1].rec_top,
        h->intra_border[1] + row_rec.ctu.i_pix_x_c + row_rec.ctu.i_ctu_w_c - 1,
        row_rec.ctu.p_frec[1] + row_rec.ctu.i_ctu_w_c - 1,
        row_rec.ctu_border[2].rec_top,
        h->intra_border[2] + row_rec.ctu.i_pix_x_c + row_rec.ctu.i_ctu_w_c - 1,
        row_rec.ctu.p_frec[2] + row_rec.ctu.i_ctu_w_c - 1,
        row_rec.ctu.i_frec[1], row_rec.ctu.i_ctu_w_c, row_rec.ctu.i_ctu_h_c);

    /* backup bottom row pixels */
    if (i_lcu_y < h->i_height_in_lcu - 1) {
      memcpy(h->intra_border[0] + row_rec.ctu.i_pix_x,
             row_rec.ctu.p_frec[0] +
                 (row_rec.ctu.i_ctu_h - 1) * h->fdec->i_stride[0],
             row_rec.ctu.i_ctu_w * sizeof(pel_t));
      memcpy(h->intra_border[1] + row_rec.ctu.i_pix_x_c,
             row_rec.ctu.p_frec[1] +
                 (row_rec.ctu.i_ctu_h_c - 1) * h->fdec->i_stride[1],
             row_rec.ctu.i_ctu_w_c * sizeof(pel_t));
      memcpy(h->intra_border[2] + row_rec.ctu.i_pix_x_c,
             row_rec.ctu.p_frec[2] +
                 (row_rec.ctu.i_ctu_h_c - 1) * h->fdec->i_stride[1],
             row_rec.ctu.i_ctu_w_c * sizeof(pel_t));
    }
  ...
  if (i_lcu_y == height_in_lcu - 1) {
    ...
    if ((h->i_frame_type == AVS2_P_SLICE || h->i_frame_type == AVS2_F_SLICE) &&
        h->b_bkgnd_picture && h->b_bkgnd_reference) {
      const int w_in_spu = h->i_width_in_spu;
      const int h_in_spu = h->i_height_in_spu;
      int x, y;
...
static int decode_lcu_row_recon(davs2_t *h, int i_lcu_y) {
  ...
  while (i_lcu_y < height_in_lcu) {
    ...
    for (i_lcu_x = 0; i_lcu_x < width_in_lcu; i_lcu_x++, lcu_xy++) {
      ...
      if (i_lcu_y > 0) {
        wait_lcu_row_reconed(h, h->fdec, i_lcu_y - 1,
                             DAVS2_MIN(i_lcu_x + 2, h->i_width_in_lcu));
      }
      ...
      decode_lcu_recon(h, &row_rec, i_lcu_level, i_lcu_x << i_lcu_level,
                       i_lcu_y << i_lcu_level);
      ...
      davs2_cache_lcu_border_uv(
          row_rec.ctu_border[1].rec_top,
          h->intra_border[1] + row_rec.ctu.i_pix_x_c + row_rec.ctu.i_ctu_w_c -
              1,
          row_rec.ctu.p_frec[1] + row_rec.ctu.i_ctu_w_c - 1,
          row_rec.ctu_border[2].rec_top,
          h->intra_border[2] + row_rec.ctu.i_pix_x_c + row_rec.ctu.i_ctu_w_c -
              1,
          row_rec.ctu.p_frec[2] + row_rec.ctu.i_ctu_w_c - 1,
          row_rec.ctu.i_frec[1], row_rec.ctu.i_ctu_w_c, row_rec.ctu.i_ctu_h_c);
    ...
    if (alf_enable && i_lcu_y) {
      alf_lcurow(h, h->p_alf->img_param, h->p_frame_alf, h->fdec,
                 i_lcu_y - 1);  // above row
      if (i_lcu_y == height_in_lcu - 1) {
        alf_lcurow(h, h->p_alf->img_param, h->p_frame_alf, h->fdec,
                   i_lcu_y);  // last row
  ...
  if (b_recon_finish) {
    ...
    // davs2_log(h, DAVS2_LOG_INFO, "POC %3d reconstruction finished.",
    // h->i_poc);
...
int decoder_alloc_extra_buffer(davs2_t *h) {
  ...
  size_t size_in_lcu = ((h->i_width + h->i_lcu_size_sub1) >> h->i_lcu_level) *
                       ((h->i_height + h->i_lcu_size_sub1) >> h->i_lcu_level);
  ...
  size_extra_frame =
      2 * davs2_frame_get_size(h->i_width, h->i_height, h->i_chroma_format, 1);
  size_extra_frame +=
      (h->b_alf + h->b_sao) *
      davs2_frame_get_size(h->i_width, h->i_height, h->i_chroma_format, 0);

  mem_size =
      sizeof(int8_t) * (w_in_spu + 16) *
          (h_in_spu + 1) + /* M1, size of intra prediction mode buffer */
      sizeof(int8_t) *
          size_in_spu + /* M3, size of prediction direction buffer */
      sizeof(ref_idx_t) *
          size_in_spu + /* M3, size of reference index (1st+2nd) buffer */
      sizeof(mv_t) * size_in_spu + /* M5, size of motion vector of 4x4 block
                                      (1st reference) buffer */
      sizeof(mv_t) * size_in_spu + /* M6, size of motion vector of 4x4 block
                                      (2nd reference) buffer */
      sizeof(uint8_t) * w_in_scu * h_in_scu *
          2 + /* M7, size of loop filter flag buffer */
      sizeof(lcu_info_t) *
          size_in_lcu + /* M8, size of SAO block parameter buffer */
      sizeof(cu_t) * h->i_size_in_scu + /* M10, size of cu_t */
      sizeof(pel_t) * h->i_width *
          3 +            /* M13, size of last LCU row bottom border */
  ...
  // AVS-S
  h->f_background_ref = davs2_frame_new(h->i_width, h->i_height,
                                        h->i_chroma_format, &mem_base, 1);
  ALIGN_POINTER(mem_base);
  h->f_background_cur = davs2_frame_new(h->i_width, h->i_height,
                                        h->i_chroma_format, &mem_base, 1);
  ALIGN_POINTER(mem_base);

  // ALF
  if (h->b_alf) {
    h->p_frame_alf = davs2_frame_new(h->i_width, h->i_height,
                                     h->i_chroma_format, &mem_base, 0);
    ALIGN_POINTER(mem_base);
  }

  // SAO
  if (h->b_sao) {
    h->p_frame_sao = davs2_frame_new(h->i_width, h->i_height,
                                     h->i_chroma_format, &mem_base, 0);
    ALIGN_POINTER(mem_base);
  }
  ...
  if ((int)mem_size < (mem_base - h->p_integral)) {
    davs2_log(h, DAVS2_LOG_ERROR,
              "No enough memory allocated. mem_size %llu <= %llu\n", mem_size,
              mem_base - h->p_integral);
    goto fail;
  }
  return 0;
...
void davs2_write_a_frame(davs2_picture_t *pic, davs2_frame_t *frame) {
  ...
  int img_height_c =
      (img_height / (frame->i_chroma_format == CHROMA_420 ? 2 : 1));
  int num_bytes_per_sample = (frame->i_output_bit_depth == 8 ? 1 : 2);
  int shift1 =
      frame->i_sample_bit_depth -
      frame->i_output_bit_depth;  // assuming "sample_bit_depth" is greater or
                                  // equal to "output_bit_depth"
  pel_t *p_src;
  uint8_t *p_dst;
  int k, j, i_src, i_dst;
  ...
  if (!shift1 && sizeof(pel_t) == num_bytes_per_sample) {
    ...
    pic->strides[2] = frame->i_stride[2] * num_bytes_per_sample;
             ...
             frame->i_output_bit_depth == 8) {  // 8bit encode -> 8bit output
    p_dst = pic->planes[0];
    i_dst = pic->strides[0];
    ...
    if (pic->num_planes == 3) {
      ...
      for (j = 0; j < img_height_c; j++) {
        ...
        p_dst += i_dst;
             ...
             frame->i_output_bit_depth == 8) {  // 10bit encode -> 8bit output
    p_dst = pic->planes[0];
    i_dst = pic->strides[0];
    p_src = frame->planes[0];
    i_src = frame->i_stride[0];
    ...
    for (j = 0; j < img_height; j++) {
      for (k = 0; k < img_width; k++) {
        p_dst[k] =
            (uint8_t)DAVS2_CLIP1((p_src[k] + (1 << (shift1 - 1))) >> shift1);
      }
...
davs2_t *decoder_open(davs2_mgr_t *mgr, davs2_t *h, int idx_decoder) {
  ...
  sprintf(h->module_log.module_name, "Dec[%2d] %06llx", idx_decoder,
 ...
 *            : es_unit - pointer to bit-stream buffer (including the following
 * parameters) :    data - pointer to bitstream buffer :    len  - data length
 * in bitstream buffer :    pts  - user pts :    dts  - user dts Return     :
 * none
 * ---------------------------------------------------------------------------
 */
void *decoder_decode_picture_data(void *arg1, int arg2) {
  ...
  if (h->b_slice_checked != 0) {
    ...
    memset(h->fdec->num_decoded_lcu_in_row, 0,
           sizeof(int) * h->i_height_in_lcu);

    // davs2_thread_mutex_lock(&mgr->mutex_aec);
    // mgr->num_active_decoders++;
    // davs2_thread_mutex_unlock(&mgr->mutex_aec);

    if (mgr->num_rec_thread &&
        davs2_threadpool_is_free((davs2_threadpool_t *)mgr->thread_pool)) {
    ...
    ///!!! make sure that all row signals of frames with 'b_refered_by_others ==
    ///1' have been set before return.
