...
const uint8_t g_partition_map_tab[] = {
  ...
  //  44   48           52   56   60   64
  PART_4x4,  PART_4x8,   255,        PART_4x16,  255, 255,
  255,       255,        255,        255,        255, 255,
  255,       255,        255,        255,  // 4
  PART_8x4,  PART_8x8,   255,        PART_8x16,  255, 255,
  255,       PART_8x32,  255,        255,        255, 255,
  255,       255,        255,        255,  // 8
  255,       255,        255,        PART_12x16, 255, 255,
  255,       255,        255,        255,        255, 255,
  255,       255,        255,        255,  // 12
  PART_16x4, PART_16x8,  PART_16x12, PART_16x16, 255, 255,
  255,       PART_16x32, 255,        255,        255, 255,
  255,       255,        255,        PART_16x64,  // 16
  255,       255,        255,        255,        255, 255,
  255,       255,        255,        255,        255, 255,
  255,       255,        255,        255,  // 20
  255,       255,        255,        255,        255, 255,
  255,       PART_24x32, 255,        255,        255, 255,
  255,       255,        255,        255,  // 24
  255,       255,        255,        255,        255, 255,
  255,       255,        255,        255,        255, 255,
  255,       255,        255,        255,  // 28
  255,       PART_32x8,  255,        PART_32x16, 255, PART_32x24,
  255,       PART_32x32, 255,        255,        255, 255,
  255,       255,        255,        PART_32x64,  // 32
  255,       255,        255,        255,        255, 255,
  255,       255,        255,        255,        255, 255,
  255,       255,        255,        255,  // 36
  255,       255,        255,        255,        255, 255,
  ...
  255,       255,        255,        255,        255, 255,
  255,       255,        255,        255,  // 60
  255,       255,        255,        PART_64x16, 255, 255,
  255,       PART_64x32, 255,        255,        255, PART_64x48,
  255,       255,        255,        PART_64x64  // 64
};

#define PIXEL_ADD_PS_C(w, h)                                          \
  static void davs2_pixel_add_ps_##w##x##h(                           \
      pel_t *a, intptr_t dstride, const pel_t *b0, const coeff_t *b1, \
...
#define BLOCKCOPY_PP_C(w, h)                                                   \
  static void davs2_blockcopy_pp_##w##x##h(pel_t *a, intptr_t stridea,         \
                                           const pel_t *b, intptr_t strideb) { \
    int x, y;                                                                  \
    for (y = 0; y < h; y++) {                                                  \
      for (x = 0; x < w; x++) {                                                \
        a[x] = b[x];                                                           \
...
#define BLOCKCOPY_SS_C(w, h)                                              \
  static void davs2_blockcopy_ss_##w##x##h(                               \
      coeff_t *a, intptr_t stridea, const coeff_t *b, intptr_t strideb) { \
    int x, y;                                                             \
    for (y = 0; y < h; y++) {                                             \
      for (x = 0; x < w; x++) {                                           \
        a[x] = b[x];                                                      \
...
#define DECL_PIXELS(cpu)                                                 \
  FUNCDEF_PU(void, pixel_avg, cpu, pel_t *dst, intptr_t dstride,         \
             const pel_t *src0, intptr_t sstride0, const pel_t *src1,    \
             intptr_t sstride1, int);                                    \
  FUNCDEF_PU(void, pixel_add_ps, cpu, pel_t *a, intptr_t dstride,        \
             const pel_t *b0, const int16_t *b1, intptr_t sstride0,      \
             intptr_t sstride1);                                         \
  FUNCDEF_PU(void, blockcopy_pp, cpu, pel_t *a, intptr_t stridea,        \
             const pel_t *b, intptr_t strideb);                          \
  FUNCDEF_PU(void, blockcopy_ss, cpu, int16_t *a, intptr_t stridea,      \
             const int16_t *b, intptr_t strideb);                        \
  FUNCDEF_CHROMA_PU(void, addAvg, cpu, const int16_t *, const int16_t *, \
                    pel_t *, intptr_t, intptr_t, intptr_t)
...
void davs2_pixel_init(uint32_t cpuid, ao_funcs_t *pixf) {
  ...
  if (cpuid & DAVS2_CPU_SSE2) {
    ...
    if (sizeof(pel_t) == sizeof(int16_t) && cpuid) {
      pixf->copy_pp[PART_64x64] =
          (copy_pp_t)davs2_blockcopy_ss_64x64_sse2; /* 64x64 */
      ...
      pixf->copy_pp[PART_32x32] =
          (copy_pp_t)davs2_blockcopy_ss_32x32_sse2; /* 32x32 */
      ...
      pixf->copy_pp[PART_16x16] =
          (copy_pp_t)davs2_blockcopy_ss_16x16_sse2; /* 16x16 */
      ...
      pixf->copy_pp[PART_8x8] =
          (copy_pp_t)davs2_blockcopy_ss_8x8_sse2; /* 8x8 */
      pixf->copy_pp[PART_8x4] = (copy_pp_t)davs2_blockcopy_ss_8x4_sse2;
      pixf->copy_pp[PART_4x8] = (copy_pp_t)davs2_blockcopy_ss_4x8_sse2;
      pixf->copy_pp[PART_4x4] =
          (copy_pp_t)davs2_blockcopy_ss_4x4_sse2; /* 4x4 */
    }
    if (sizeof(coeff_t) == sizeof(int16_t) && cpuid) {
      pixf->copy_ss[PART_64x64] =
          (copy_ss_t)davs2_blockcopy_ss_64x64_sse2; /* 64x64 */
      ...
      pixf->copy_ss[PART_32x32] =
          (copy_ss_t)davs2_blockcopy_ss_32x32_sse2; /* 32x32 */
      ...
      pixf->copy_ss[PART_16x16] =
          (copy_ss_t)davs2_blockcopy_ss_16x16_sse2; /* 16x16 */
      ...
      pixf->copy_ss[PART_8x8] =
          (copy_ss_t)davs2_blockcopy_ss_8x8_sse2; /* 8x8 */
      pixf->copy_ss[PART_8x4] = (copy_ss_t)davs2_blockcopy_ss_8x4_sse2;
      pixf->copy_ss[PART_4x8] = (copy_ss_t)davs2_blockcopy_ss_4x8_sse2;
      pixf->copy_ss[PART_4x4] =
          (copy_ss_t)davs2_blockcopy_ss_4x4_sse2; /* 4x4 */
    }
#else
