...
/* ---------------------------------------------------------------------------
 */
static void alf_recon_coefficients(alf_param_t *p_alf_param,
                                   int p_filter_coeff[][ALF_MAX_NUM_COEF]) {
  ...
  for (j = 0; j < p_alf_param->filters_per_group; j++) {
    ...
    p_filter_coeff[j][num_coeff] =
        (alf_num - sum) + p_alf_param->coeffmulti[j][num_coeff];
  }
}
...
static void alf_init_var_table(alf_param_t *p_alf_param, int *p_var_tab) {
  if (p_alf_param->filters_per_group > 1) {
    ...
    for (i = 1; i < ALF_NUM_VARS; ++i) {
      p_var_tab[i] = (p_alf_param->filterPattern[i]) ? (p_var_tab[i - 1] + 1)
                                                     : p_var_tab[i - 1];
    }
  } else {
...
/* ---------------------------------------------------------------------------
 */
static void alf_filter_block1(pel_t *p_dst, const pel_t *p_src, int stride,
                              int lcu_pix_x, int lcu_pix_y, int lcu_width,
                              int lcu_height, int *alf_coeff, int b_top_avail,
                              int b_down_avail) {
...

  {
    ...
    int endPos =
        b_down_avail ? (lcu_pix_y + lcu_height - 4) : (lcu_pix_y + lcu_height);
...
/* ---------------------------------------------------------------------------
 */
static void alf_filter_block2(pel_t *p_dst, const pel_t *p_src, int i_src,
                              int lcu_pix_x, int lcu_pix_y, int lcu_width,
                              int lcu_height, int *alf_coeff, int b_top_avail,
                              int b_down_avail) {
...
/* ---------------------------------------------------------------------------
 */
static void deriveBoundaryAvail(davs2_t *h, int lcu_xy, int width_in_lcu,
                                int height_in_lcu, int *b_top_avail,
                                int *b_down_avail) {
  ...
  *b_down_avail = (lcu_xy < (height_in_lcu - 1) * width_in_lcu);
  ...
  if (!h->seq_info.cross_loop_filter_flag) {
    ...
    int scu_xy = (lcu_pic_y >> MIN_CU_SIZE_IN_BIT) * width_in_scu +
                 (lcu_pic_x >> MIN_CU_SIZE_IN_BIT);
    // int scu_xy_next_row = scu_xy + (1 << (h->i_lcu_level -
    // MIN_CU_SIZE_IN_BIT)) * width_in_scu;
    int slice_idx_top =
        *b_top_avail ? h->scu_data[scu_xy - width_in_scu].i_slice_nr : -1;
    // int slice_idx_down = *b_down_avail ?
    // h->scu_data[scu_xy_next_row].i_slice_nr : -1;
...
void alf_init_buffer(davs2_t *h) {
  static const uint8_t regionTable[ALF_NUM_VARS] = { 0,  1,  4,  5,  15, 2,
                                                     3,  6,  14, 11, 10, 7,
                                                     13, 12, 9,  8 };
  ...
  for (j = 0; j < height_in_lcu; j++) {
    ...
    for (i = 0; i < width_in_lcu; i++) {
      ...
      h->p_alf->tab_lcu_region[j * width_in_lcu + i] =
          regionTable[region_idx_y * 4 + region_idx_x];
    }
  }
 ...
 * ALF one LCU block
 */
static void alf_lcu_block(davs2_t *h, alf_param_t *p_alf_param,
                          davs2_frame_t *p_tmp_frm, davs2_frame_t *p_dec_frm,
  ...
  int lcu_width =
      (lcu_pix_x + lcu_size > img_width) ? (img_width - lcu_pix_x) : lcu_size;
  int lcu_height =
      (lcu_pix_y + lcu_size > img_height) ? (img_height - lcu_pix_y) : lcu_size;
  ...
  deriveBoundaryAvail(h, lcu_xy, width_in_lcu, height_in_lcu, &b_top_avail,
                      &b_down_avail);
  ...
  if (h->lcu_infos[lcu_xy].enable_alf[0]) {
    ...
    alf_coef =
        h->p_alf
            ->filterCoeffSym[h->p_alf->tab_region_coeff_idx[lcu_region_idx]];

    gf_davs2.alf_block[0](p_dec_frm->planes[0], p_tmp_frm->planes[0],
                          p_dec_frm->i_stride[0], lcu_pix_x, lcu_pix_y,
                          lcu_width, lcu_height, alf_coef, b_top_avail,
                          b_down_avail);
    gf_davs2.alf_block[1](p_dec_frm->planes[0], p_tmp_frm->planes[0],
                          p_dec_frm->i_stride[0], lcu_pix_x, lcu_pix_y,
                          lcu_width, lcu_height, alf_coef, b_top_avail,
                          b_down_avail);
  }
  ...
  if (h->lcu_infos[lcu_xy].enable_alf[1]) {
    ...
    gf_davs2.alf_block[0](p_dec_frm->planes[1], p_tmp_frm->planes[1],
                          p_dec_frm->i_stride[1], lcu_pix_x, lcu_pix_y,
                          lcu_width, lcu_height, alf_coef, b_top_avail,
                          b_down_avail);
    gf_davs2.alf_block[1](p_dec_frm->planes[1], p_tmp_frm->planes[1],
                          p_dec_frm->i_stride[1], lcu_pix_x, lcu_pix_y,
                          lcu_width, lcu_height, alf_coef, b_top_avail,
                          b_down_avail);
  }
  ...
  if (h->lcu_infos[lcu_xy].enable_alf[2]) {
    ...
    gf_davs2.alf_block[0](p_dec_frm->planes[2], p_tmp_frm->planes[2],
                          p_dec_frm->i_stride[2], lcu_pix_x, lcu_pix_y,
                          lcu_width, lcu_height, alf_coef, b_top_avail,
                          b_down_avail);
    gf_davs2.alf_block[1](p_dec_frm->planes[2], p_tmp_frm->planes[2],
                          p_dec_frm->i_stride[2], lcu_pix_x, lcu_pix_y,
                          lcu_width, lcu_height, alf_coef, b_top_avail,
                          b_down_avail);
  }
...
/* ---------------------------------------------------------------------------
 */
void alf_lcurow(davs2_t *h, alf_param_t *p_alf_param, davs2_frame_t *p_tmp_frm,
                davs2_frame_t *p_dec_frm, int i_lcu_y) {
