...
/* ---------------------------------------------------------------------------
 */
static ALWAYS_INLINE void check_scaling_neighbor_mv(davs2_t *h, mv_t *mv,
                                                    int mult_distance,
                                                    int ref_neighbor) {
  if (ref_neighbor >= 0) {
    ...
    mv->y = scale_mv_default_y(h, mv->y, mult_distance, devide_distance,
                               devide_distance_src);
...
/* ---------------------------------------------------------------------------
 */
static ALWAYS_INLINE void check_scaling_neighbor_mv_b(davs2_t *h, mv_t *mv,
                                                      int mult_distance,
                                                      int mult_distance_src,
                                                      int ref_neighbor) {
  if (ref_neighbor >= 0) {
    mv->y = scale_mv_default_y(h, mv->y, mult_distance, mult_distance,
                               mult_distance_src);
...
/* ---------------------------------------------------------------------------
 */
static ALWAYS_INLINE int recheck_neighbor_ref_avail(davs2_t *h, int ref_frame,
                                                    int neighbor_ref) {
  if (neighbor_ref != -1) {
    if (((ref_frame == h->num_of_references - 1 &&
          neighbor_ref != h->num_of_references - 1) ||
         (ref_frame != h->num_of_references - 1 &&
          neighbor_ref == h->num_of_references - 1)) &&
        (h->i_frame_type == AVS2_P_SLICE || h->i_frame_type == AVS2_F_SLICE) &&
        h->b_bkgnd_picture) {
      neighbor_ref = -1;
    }
...
/* ---------------------------------------------------------------------------
 */
static ALWAYS_INLINE int derive_mv_pred_type(int ref_frame, int rFrameL,
                                             int rFrameU, int rFrameUR,
                                             int pu_type_for_mvp) {
  int mvp_type = MVPRED_xy_MIN;

  if ((rFrameL != INVALID_REF) && (rFrameU == INVALID_REF) &&
      (rFrameUR == INVALID_REF)) {
    mvp_type = MVPRED_L;
  } else if ((rFrameL == INVALID_REF) && (rFrameU != INVALID_REF) &&
             (rFrameUR == INVALID_REF)) {
    mvp_type = MVPRED_U;
  } else if ((rFrameL == INVALID_REF) && (rFrameU == INVALID_REF) &&
             (rFrameUR != INVALID_REF)) {
...
static ALWAYS_INLINE int16_t derive_median_mv(int mva, int mvb, int mvc) {
  ...
  if (((mva < 0) && (mvb > 0) && (mvc > 0)) ||
      ((mva > 0) && (mvb < 0) && (mvc < 0))) {
    mvp = (mvb + mvc) / 2;  // b
  } else if (((mvb < 0) && (mva > 0) && (mvc > 0)) ||
             ((mvb > 0) && (mva < 0) && (mvc < 0))) {
    mvp = (mvc + mva) / 2;  // c
  } else if (((mvc < 0) && (mva > 0) && (mvb > 0)) ||
             ((mvc > 0) && (mva < 0) && (mvb < 0))) {
    mvp = (mva + mvb) / 2;  // a
  } else {
 ...
 */
static ALWAYS_INLINE void cu_get_neighbors_default_mvp(davs2_t *h, cu_t *p_cu,
                                                       int pix_cu_x,
                                                       int pix_cu_y, int bsx) {
  ...
  int avail_TR =
      h->p_tab_TR_avail[(y4_TR << (h->i_lcu_level - B4X4_IN_BIT)) + x4_TR];
  ...
  cu_get_neighbor_spatial(h, cur_slice_idx, &neighbors[BLK_TOPRIGHT],
                          avail_TR ? x1 + 1 : -1, y0 - 1);
}

/* ---------------------------------------------------------------------------
 * set motion vector predictor
 */
void get_mvp_default(davs2_t *h, cu_t *p_cu, int pix_x, int pix_y, mv_t *pmv,
                     int bwd_2nd, int ref_frame, int bsx, int pu_type_for_mvp) {
  ...
  rFrameUR = is_available_UR
                 ? h->lcu.neighbor_inter[BLK_TOPRIGHT].ref_idx.r[bwd_2nd]
                 : rFrameUL;
  ...
  mvPredType = derive_mv_pred_type(ref_frame, rFrameL, rFrameU, rFrameUR,
                                   pu_type_for_mvp);

  if (h->i_frame_type == AVS2_B_SLICE) {
    int mult_distance = get_distance_index_b(h, bwd_2nd ? B_BWD : B_FWD);
    int mult_distance_src =
        get_distance_index_b_scale(h, bwd_2nd ? B_BWD : B_FWD);
    check_scaling_neighbor_mv_b(h, &mva, mult_distance, mult_distance_src,
                                rFrameL);
    check_scaling_neighbor_mv_b(h, &mvb, mult_distance, mult_distance_src,
                                rFrameU);
    check_scaling_neighbor_mv_b(h, &mvc, mult_distance, mult_distance_src,
                                rFrameUR);
  } else {
  ...
  switch (mvPredType) {
    case MVPRED_xy_MIN:
      ...
      break;
    case MVPRED_L: pmv->v = mva.v; break;
    case MVPRED_U: pmv->v = mvb.v; break;
...
/* ---------------------------------------------------------------------------
 */
static void get_mv_bskip_spatial(davs2_t *h, mv_t *fw_pmv, mv_t *bw_pmv,
                                 int num_skip_dir) {
  ...
  memset(h->lcu.mv_tskip_1st, 0,
         sizeof(h->lcu.mv_tskip_1st) + sizeof(h->lcu.mv_tskip_2nd));
...
/* ---------------------------------------------------------------------------
 */
static ALWAYS_INLINE void get_mv_pf_skip_temporal(davs2_t *h, mv_t *p_mv,
                                                  int block_offset,
                                                  int cur_dist) {
...
/* ---------------------------------------------------------------------------
 */
static void fill_mv_pf_skip_temporal(davs2_t *h, cu_t *p_cu, int pix_x,
                                     int pix_y, int cu_size) {
  ...
  ref_idx.r[0] = 0;
  ref_idx.r[1] =
      (int8_t)(p_cu->i_weighted_skipmode != 0 ? p_cu->i_weighted_skipmode
                                              : INVALID_REF);
  ...
  for (i = 0; i < p_cu->num_pu; i++) {
    ...
    if (ref_idx.r[1] != INVALID_REF) {
      ...
      mv_2nd.y = scale_mv_skip_y(h, mv_1st.y, delta[ref_idx.r[1]], delta[0],
                                 delta_src[0]);
    } else {
      mv_2nd.v = 0;
    }
...
/* ---------------------------------------------------------------------------
 */
static INLINE void get_mv_fskip_spatial(davs2_t *h) {
  neighbor_inter_t *p_neighbors = h->lcu.neighbor_inter;
  int bid_flag = 0, fw_flag = 0, bid2 = 0, fw2 = 0;
  int j;

  memset(h->lcu.ref_skip_1st, 0,
         sizeof(h->lcu.ref_skip_1st) + sizeof(h->lcu.ref_skip_2nd) +
             sizeof(h->lcu.mv_tskip_1st) + sizeof(h->lcu.mv_tskip_2nd));

  for (j = 0; j < 6; j++) {
    if (p_neighbors[j].ref_idx.r[0] != -1 &&
        p_neighbors[j].ref_idx.r[1] != -1) {  // bid
    ...
    } else if (p_neighbors[j].ref_idx.r[0] != -1 &&
               p_neighbors[j].ref_idx.r[1] == -1) {  // fw
...
/* ---------------------------------------------------------------------------
 */
static void fill_mv_bskip(davs2_t *h, cu_t *p_cu, int pix_x, int pix_y,
                          int size_in_scu) {
  int width_in_spu = h->i_width_in_spu;
  ...
  if (ds_mode != DS_NONE) {
    ...
    int r, c;
    int cu_size_in_spu = size_in_scu
                         << (MIN_CU_SIZE_IN_BIT - MIN_PU_SIZE_IN_BIT);
    ref_idx_t ref_idx;
    int8_t i_dir_pred;
 ...
 * 包括设置参考帧索引和运动矢量
 */
void fill_mv_and_ref_for_skip(davs2_t *h, cu_t *p_cu, int pix_x, int pix_y,
                              int size_in_scu) {
  assert(p_cu->i_cu_type == PRED_SKIP);

  if (h->i_frame_type == AVS2_B_SLICE) {
    fill_mv_bskip(h, p_cu, pix_x, pix_y, size_in_scu);
  } else if ((h->i_frame_type == AVS2_F_SLICE) ||
             (h->i_frame_type == AVS2_P_SLICE)) {
    if (p_cu->i_md_directskip_mode == 0) {
      fill_mv_pf_skip_temporal(h, p_cu, pix_x, pix_y,
                               size_in_scu << MIN_CU_SIZE_IN_BIT);
    } else {
      int width_in_spu = h->i_width_in_spu;
      int block_offset = (pix_y >> MIN_PU_SIZE_IN_BIT) * width_in_spu +
                         (pix_x >> MIN_PU_SIZE_IN_BIT);
      ref_idx_t *p_ref_1st = h->p_ref_idx + block_offset;
