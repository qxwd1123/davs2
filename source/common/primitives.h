 ...
 */
typedef void (*block_copy_pp_t)(pel_t *dst, intptr_t i_dst, pel_t *src,
                                intptr_t i_src, int w, int h);
typedef void (*block_copy_sc_t)(coeff_t *dst, intptr_t i_dst, int16_t *src,
                                intptr_t i_src, int w, int h);
typedef void (*block_intpl_t)(const pel_t *src, intptr_t srcStride, pel_t *dst,
                              intptr_t dstStride, int coeffIdx);
typedef void (*block_intpl_ext_t)(const pel_t *src, intptr_t srcStride,
                                  pel_t *dst, intptr_t dstStride, int coeffIdxX,
                                  int coeffIdxY);
typedef void (*intpl_t)(pel_t *dst, int i_dst, pel_t *src, int i_src, int width,
                        int height, const int8_t *coeff);
typedef void (*intpl_ext_t)(pel_t *dst, int i_dst, pel_t *src, int i_src,
                            int width, int height, const int8_t *coeff_x,
                            const int8_t *coeff_y);
typedef void (*pixel_avg_pp_t)(pel_t *dst, int i_dst, const pel_t *src0,
                               int i_src0, const pel_t *src1, int i_src1,
                               int width, int height);
typedef void (*dct_t)(const coeff_t *src, coeff_t *dst, int i_src);

typedef void (*intra_pred_t)(pel_t *src, pel_t *dst, int i_dst, int dir_mode,
                             int bsx, int bsy);
typedef void (*fill_edge_t)(const pel_t *p_topleft, int i_topleft,
                            const pel_t *p_lcu_ep, pel_t *EP, uint32_t i_avail,
                            int bsx, int bsy);

typedef void *(*memcpy_t)(void *dst, const void *src, size_t n);
typedef void (*copy_pp_t)(pel_t *dst, intptr_t dstStride, const pel_t *src,
                          intptr_t srcStride);  // dst is aligned
typedef void (*copy_ss_t)(coeff_t *dst, intptr_t dstStride, const coeff_t *src,
                          intptr_t srcStride);

typedef void (*pixel_add_ps_t)(pel_t *dst, intptr_t dstride, const pel_t *b0,
                               const coeff_t *b1, intptr_t sstride0,
                               intptr_t sstride1);

typedef void (*lcu_deblock_t)(davs2_t *h, davs2_frame_t *frm, int i_lcu_x,
                              int i_lcu_y);

typedef void (*sao_flt_bo_t)(pel_t *p_dst, int i_dst, const pel_t *p_src,
                             int i_src, int i_block_w, int i_block_h,
                             int bit_depth, const sao_param_t *sao_param);
typedef void (*sao_flt_eo_t)(pel_t *p_dst, int i_dst, const pel_t *p_src,
                             int i_src, int i_block_w, int i_block_h,
...
typedef struct ao_funcs_t {
  ...
  /* plane copy */
  void (*plane_copy)(pel_t *dst, intptr_t i_dst, pel_t *src, intptr_t i_src,
                     int w, int h);
...
#endif
  intpl_t intpl_luma_ver[2][3];  //[2]:根据块大小进行函数拆分（0:size<=8
                                 //1:size>=16）     [3]:根据权重系数进行拆分
  ...
  void (*deblock_luma[2])(pel_t *src, int stride, int alpha, int beta,
                          uint8_t *flt_flag);
#if HDR_CHROMA_DELTA_QP
  void (*deblock_chroma[2])(pel_t *src_u, pel_t *src_v, int stride, int *alpha,
                            int *beta, uint8_t *flt_flag);
#else
  void (*deblock_chroma[2])(pel_t *src_u, pel_t *src_v, int stride, int alpha,
                            int beta, uint8_t *flt_flag);
#endif
  ...
  void (*inv_transform_2nd)(coeff_t *coeff, int i_coeff, int i_mode, int b_top,
                            int b_left);

  /* quant */
  void (*dequant)(coeff_t *coef, const int i_coef, const int scale,
                  const int shift);
} ao_funcs_t;
