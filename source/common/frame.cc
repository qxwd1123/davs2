...
/* ---------------------------------------------------------------------------
 */
static ALWAYS_INLINE void pad_line_pixel(pel_t *pix, int width, int num_pad) {
...
void pad_line_lcu(davs2_t *h, int lcu_y) {
  ...
  for (i = 0; i < 3; i++) {
    int chroma_shift = !!i;
    int start =
        ((lcu_y + 0) << h->i_lcu_level) >> chroma_shift;  ///< -4 for ALF
...
/* ---------------------------------------------------------------------------
 */
static ALWAYS_INLINE int align_stride(int x, int align, int disalign) {
...
/* ---------------------------------------------------------------------------
 */
static ALWAYS_INLINE int align_plane_size(int x, int disalign) {
  if (!(x & (disalign - 1))) {
    x += 128;
  }
...
size_t davs2_frame_get_size(int width, int height, int chroma_format,
  ...
  const int max_lcu_height =
      (height + (1 << 4) - 1) >> 4; /* frame height in 16x16 LCU */
  const int align = 32;
  ...
  size_l = align_plane_size(
      stride_l * (height + AVS2_PAD * 2) + CACHE_LINE_SIZE, disalign);
  size_c = align_plane_size(stride_c * (height_c + AVS2_PAD) + CACHE_LINE_SIZE,
                            disalign);

  /* compute space size and alloc memory */
  mem_size =
      sizeof(davs2_frame_t) + /* M0, size of frame handle */
      sizeof(pel_t) *
          (size_l + size_c * 2) + /* M1, size of planes buffer: Y+U+V */
      sizeof(int8_t) *
          extra_buf_size + /* M2, size of SPU reference index buffer */
      sizeof(mv_t) * extra_buf_size + /* M3, size of SPU motion vector buffer */
      sizeof(davs2_thread_cond_t) *
          max_lcu_height + /* M4, condition variables for each LCU line */
...
davs2_frame_t *davs2_frame_new(int width, int height, int chroma_format,
  ...
  const int height_in_spu = height >> MIN_PU_SIZE_IN_BIT;
  const int max_lcu_height =
      (height + (1 << 4) - 1) / (1 << 4); /* frame height in 16x16 LCU */
  const int align = 32;
  const int disalign = 1 << 16;
  int extra_buf_size = 0; /* extra buffer size */
  int stride_l, stride_c;
...
void davs2_frame_copy_planes(davs2_frame_t *p_dst, davs2_frame_t *p_src) {
  ...
  gf_davs2.memcpy_aligned(
      p_dst->planes[0], p_src->planes[0],
      p_src->i_stride[0] * p_src->i_lines[0] * sizeof(pel_t));
  gf_davs2.memcpy_aligned(
      p_dst->planes[1], p_src->planes[1],
      p_src->i_stride[1] * p_src->i_lines[1] * sizeof(pel_t));
  gf_davs2.memcpy_aligned(
      p_dst->planes[2], p_src->planes[2],
      p_src->i_stride[2] * p_src->i_lines[2] * sizeof(pel_t));
#else
  gf_davs2.plane_copy(p_dst->planes[0], p_dst->i_stride[0], p_src->planes[0],
                      p_src->i_stride[0], p_src->i_width[0], p_src->i_lines[0]);
  gf_davs2.plane_copy(p_dst->planes[1], p_dst->i_stride[1], p_src->planes[1],
                      p_src->i_stride[1], p_src->i_width[1], p_src->i_lines[1]);
  gf_davs2.plane_copy(p_dst->planes[2], p_dst->i_stride[2], p_src->planes[2],
                      p_src->i_stride[2], p_src->i_width[2], p_src->i_lines[2]);
#endif
}
...
void davs2_frame_copy_lcurow(davs2_t *h, davs2_frame_t *p_dst,
  ...
  pix_y = (i_lcu_y << (h->i_lcu_level - 1)) + pix_offset;
  lcu_h = DAVS2_MIN(h->i_height >> 1, ((i_lcu_y + 1) << (h->i_lcu_level - 1))) -
          pix_y;
