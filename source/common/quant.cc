...
/* ---------------------------------------------------------------------------
 */
const int16_t wq_param_default[2][6] = { { 67, 71, 71, 80, 80, 106 },
                                         { 64, 49, 53, 58, 58, 64 } };

/* ---------------------------------------------------------------------------
 */
static const int g_WqMDefault4x4[16] = { 64, 64, 64, 68, 64, 64, 68, 72,
...
static const int g_WqMDefault8x8[64] = {
  ...
  64, 64,  68,  72,  76,  80,  88,  100, 64,  68,  72,  80,  84,  92,  100, 28,
...
void wq_init_frame_quant_param(davs2_t *h) {
  ...
  for (k = 0; k < 2; k++) {
    for (j = 0; j < 8; j++) {
      for (i = 0; i < 8; i++) {
        p->wq_matrix[1][k][j * 8 + i] =
            p->wquant_param[k][WeightQuantModel[p->wq_model][j * 8 + i]];
      }
    }
  }
  ...
  for (k = 0; k < 2; k++) {
    for (j = 0; j < 4; j++) {
      for (i = 0; i < 4; i++) {
        p->wq_matrix[0][k][j * 4 + i] =
            p->wquant_param[k][WeightQuantModel4x4[p->wq_model][j * 4 + i]];
      }
    }
  }
}
...
void wq_update_frame_matrix(davs2_t *h) {
  ...
  for (uiWQMSizeId = 0; uiWQMSizeId < 4; uiWQMSizeId++) {
    ...
    if (p->pic_wq_data_index == 0) {
      ...
      if (p->wq_param == 0) {
        for (i = 0; i < (uiBlockSize * uiBlockSize); i++) {
          p->cur_wq_matrix[uiWQMSizeId][i] =
              p->wq_matrix[uiWMQId][DETAILED][i];  // detailed weighted matrix
        }
      } else if (p->wq_param == 1) {
        for (i = 0; i < (uiBlockSize * uiBlockSize); i++) {
          p->cur_wq_matrix[uiWQMSizeId][i] =
              p->wq_matrix[uiWMQId][0][i];  // undetailed weighted matrix
        }
      }
      ...
      if (p->wq_param == 2) {
        for (i = 0; i < (uiBlockSize * uiBlockSize); i++) {
          p->cur_wq_matrix[uiWQMSizeId][i] =
              p->wq_matrix[uiWMQId][1][i];  // detailed weighted matrix
        }
      }
...
/* ---------------------------------------------------------------------------
 */
static void dequant_c(coeff_t *p_coeff, const int i_coef, const int scale,
                      const int shift) {
  ...
  for (i = 0; i < i_coef; i++) {
    if (p_coeff[i]) {
      p_coeff[i] = (coeff_t)DAVS2_CLIP3(-32768, 32767,
                                        (p_coeff[i] * scale + add) >> shift);
    }
  }
}
...
/* ---------------------------------------------------------------------------
 */
static void dequant_weighted_c(coeff_t *p_coeff, int i_coeff, int bsx, int bsy,
                               int scale, int shift, int16_t *wq_matrix,
  ...
  for (j = 0; j < bsy; j++) {
    for (i = 0; i < bsx; i++) {
      ...
      if (p_coeff[i]) {
        int cur_coeff =
            (((((p_coeff[i] * wqm_coef) >> wqm_shift) * scale) >> 4) + add) >>
            shift;
 ...
 */
void dequant_coeffs(davs2_t *h, coeff_t *p_coeff, int bsx, int bsy, int scale,
                    int shift, int WQMSizeId) {
  if (h->seq_info.enable_weighted_quant) {
    int wqm_shift = (h->wq.pic_wq_data_index == 1) ? 3 : 0;
    dequant_weighted_c(p_coeff, bsx, bsx, bsy, scale, shift,
                       h->wq.cur_wq_matrix[WQMSizeId], wqm_shift, WQMSizeId);
  } else {
