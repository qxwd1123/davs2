...
/* Based on the agreed standing that davs2 decoder does not need to utilize >64
 * logical cpus, this API does not detect nor utilize more than 64 cpus for
...
/* number of times to spin a thread about to block on a locked mutex before
 * retrying and sleeping if still locked */
...
typedef BOOL(WINAPI *cond_wait_t)(davs2_thread_cond_t *cond,
...
int davs2_thread_create(davs2_thread_t *thread, const davs2_thread_attr_t *attr,
  ...
  thread->arg = arg;
  thread->handle = (void *)_beginthreadex(NULL, 0, davs2_win32thread_worker,
                                          thread, 0, NULL);
...
int davs2_win32_threading_init(void) {
  ...
  thread_control.cond_init =
      (cond_func_t)GetProcAddress(kernel_dll, "InitializeConditionVariable");
  if (thread_control.cond_init) {
    /* we're on a windows 6.0+ kernel, acquire the rest of the functions */
    thread_control.cond_broadcast =
        (cond_func_t)GetProcAddress(kernel_dll, "WakeAllConditionVariable");
    thread_control.cond_signal =
        (cond_func_t)GetProcAddress(kernel_dll, "WakeConditionVariable");
    thread_control.cond_wait =
        (cond_wait_t)GetProcAddress(kernel_dll, "SleepConditionVariableCS");
  }
...
int davs2_thread_num_processors_np() {
  ...
  /* GetProcessAffinityMask returns affinities of 0 when the process has threads
   * in multiple processor groups. On platforms that support processor grouping,
   * use GetThreadGroupAffinity to get the current thread's affinity instead. */
#if ARCH_X86_64
  /* find function pointers to API functions specific to x86_64 platforms, if
   * they exist. BOOL GetThreadGroupAffinity(_In_  HANDLE hThread, _Out_
   * PGROUP_AFFINITY GroupAffinity); */
  typedef BOOL (*get_thread_affinity_t)(
      HANDLE thread, davs2_group_affinity_t * group_affinity);
  HMODULE kernel_dll = GetModuleHandle(TEXT("kernel32.dll"));
  get_thread_affinity_t get_thread_affinity =
      (get_thread_affinity_t)GetProcAddress(kernel_dll,
