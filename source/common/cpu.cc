...
#define SSE2 MMX2 | DAVS2_CPU_SSE | DAVS2_CPU_SSE2
  ...
  { "SSE4.2", SSE2 | DAVS2_CPU_SSE3 | DAVS2_CPU_SSSE3 | DAVS2_CPU_SSE4 |
                  DAVS2_CPU_SSE42 },
#define AVX                                                                    \
  SSE2 | DAVS2_CPU_SSE3 | DAVS2_CPU_SSSE3 | DAVS2_CPU_SSE4 | DAVS2_CPU_SSE42 | \
      DAVS2_CPU_AVX
  { "AVX", AVX },
...
char *davs2_get_simd_capabilities(char *buf, uint32_t cpuid) {
  ...
  for (int i = 0; davs2_cpu_names[i].flags; i++) {
    if (!strcmp(davs2_cpu_names[i].name, "SSE") && (cpuid & DAVS2_CPU_SSE2))
      continue;
    ...
    if (!strcmp(davs2_cpu_names[i].name, "SSE3") &&
      ...
      continue;
    if (!strcmp(davs2_cpu_names[i].name, "SSE4.1") && (cpuid & DAVS2_CPU_SSE42))
      continue;
    if (!strcmp(davs2_cpu_names[i].name, "BMI1") && (cpuid & DAVS2_CPU_BMI2))
      continue;
    if ((cpuid & davs2_cpu_names[i].flags) ==
            (uint32_t)davs2_cpu_names[i].flags &&
...
uint32_t davs2_cpu_detect(void) {
  ...
  if (max_extended_cap >= 0x80000001) {
    ...
    if (ecx & 0x00000020)
      cpuid |=
          DAVS2_CPU_LZCNT;  /* Supported by Intel chips starting with Haswell */
    if (ecx & 0x00000040) { /* SSE4a, AMD only */
      int family = ((eax >> 8) & 0xf) + ((eax >> 20) & 0xff);
      cpuid |= DAVS2_CPU_SSE2_IS_FAST; /* Phenom and later CPUs have fast SSE
                                          units */
      if (family == 0x14) {
        cpuid &= ~DAVS2_CPU_SSE2_IS_FAST; /* SSSE3 doesn't imply fast SSE
                                             anymore... */
        cpuid |= DAVS2_CPU_SSE2_IS_SLOW;  /* Bobcat has 64-bit SIMD units */
        cpuid |=
            DAVS2_CPU_SLOW_PALIGNR; /* palignr is insanely slow on Bobcat */
      }
      if (family == 0x16) {
        cpuid |=
            DAVS2_CPU_SLOW_PSHUFB; /* Jaguar's pshufb isn't that slow, but it's
                                    * slow enough compared to alternate
                                    * instruction sequences that this is equal
                                    * or faster on almost all such functions. */
      }
    }
    ...
    if (!strcmp((char *)vendor, "AuthenticAMD")) {
      ...
      if ((cpuid & DAVS2_CPU_SSE2) && !(cpuid & DAVS2_CPU_SSE2_IS_FAST)) {
        cpuid |= DAVS2_CPU_SSE2_IS_SLOW; /* AMD CPUs come in two types: terrible
                                            at SSE and great at it */
      }
    }
  }
  ...
  if (!strcmp((char *)vendor, "GenuineIntel")) {
    ...
    if (family == 6) {
      /* 6/9 (pentium-m "banias"), 6/13 (pentium-m "dothan"), and 6/14 (core1
       * "yonah") theoretically support sse2, but it's significantly slower than
       * mmx for almost all of x264's functions, so let's just pretend they
       * don't. */
      if (model == 9 || model == 13 || model == 14) {
        cpuid &= ~(DAVS2_CPU_SSE2 | DAVS2_CPU_SSE3);
        // DAVS2_CHECK(!(cpuid & (DAVS2_CPU_SSSE3 | DAVS2_CPU_SSE4)),
        // "unexpected CPU ID %d\n", cpuid);
      } else if (model == 28) {
        /* Detect Atom CPU */
        ...
        cpuid |= DAVS2_CPU_SLOW_PSHUFB;
                 ...
                 model < 23) {
        /* Conroe has a slow shuffle unit. Check the model number to make sure
         * not to include crippled low-end Penryns and Nehalems that don't have
         * SSE4. */
  ...
  if ((!strcmp((char *)vendor, "GenuineIntel") ||
       !strcmp((char *)vendor, "CyrixInstead")) &&
      !(cpuid & DAVS2_CPU_SSE42)) {
    ...
    if (!cache && max_basic_cap >= 2) {
      ...
      static const uint8_t cache32_ids[] = { 0x0a, 0x0c, 0x41, 0x42, 0x43, 0x44,
                                             0x45, 0x82, 0x83, 0x84, 0x85, 0 };
      static const uint8_t cache64_ids[] = { 0x22, 0x23, 0x25, 0x29, 0x2c, 0x46,
                                             0x47, 0x49, 0x60, 0x66, 0x67, 0x68,
                                             0x78, 0x79, 0x7a, 0x7b, 0x7c, 0x7c,
                                             0x7f, 0x86, 0x87, 0 };
      uint32_t buf[4];
      int max, i = 0, j;
      do {
    ...
    if (cache == 32) {
      ...
      davs2_log(NULL, DAVS2_LOG_WARNING,
