...
/* ---------------------------------------------------------------------------
 */
static void davs2_reconfigure_decoder(davs2_mgr_t *h) { UNUSED_PARAMETER(h); }

/* ---------------------------------------------------------------------------
 * sequence header
 */
static int parse_sequence_header(davs2_mgr_t *mgr, davs2_seq_t *seq,
                                 davs2_bs_t *bs) {
  static const float FRAME_RATE[8] = { 24000.0f / 1001.0f, 24.0f, 25.0f,
                                       30000.0f / 1001.0f, 30.0f, 50.0f,
                                       60000.0f / 1001.0f, 60.0f };
  rps_t *p_rps = NULL;
  ...
  if (seq->head.chroma_format != CHROMA_420 &&
      seq->head.chroma_format != CHROMA_400) {
    return -1;
  }
  if (seq->head.chroma_format == CHROMA_400) {
    davs2_log(mgr, DAVS2_LOG_WARNING,
              "Un-supported Chroma Format YUV400 as 0 for GB/T.\n");
  }
  ...
  u_v(bs, 1, "marker bit");
  seq->b_temporal_id_exist =
      u_flag(bs, "temporal_id exist flag");  // get Extension Flag
        ...
        for (y = 0; y < uiWqMSize; y++) {
          for (x = 0; x < uiWqMSize; x++) {
            seq->seq_wq_matrix[sizeId][y * uiWqMSize + x] =
                (int16_t)ue_v(bs, "weight_quant_coeff");
          }
        }
  ...
  seq->enable_background_picture =
      u_flag(bs, "background_picture_disable") ^ 0x01;
  ...
  if (1 != u_v(bs, 1, "marker bit")) {
    davs2_log(mgr, DAVS2_LOG_ERROR,
              "expected marker_bit 1 while received 0, FILE %s, Row %d\n",
              __FILE__, __LINE__);
  }
  ...
  seq->i_enc_width = ((seq->head.width + MIN_CU_SIZE - 1) >> MIN_CU_SIZE_IN_BIT)
                     << MIN_CU_SIZE_IN_BIT;
  seq->i_enc_height =
      ((seq->head.height + MIN_CU_SIZE - 1) >> MIN_CU_SIZE_IN_BIT)
      << MIN_CU_SIZE_IN_BIT;
  seq->valid_flag = 1;
 ...
 */
static INLINE void deblock_init_frame_parames(davs2_t *h) {
  ...
  h->alpha_c = ALPHA_TABLE[DAVS2_CLIP3(0, 63, QP_c + h->i_alpha_offset)]
               << shift;
...
static int parse_picture_header_intra(davs2_t *h, davs2_bs_t *bs) {
  ...
  if (predict) {
    ...
    assert((unsigned int)h->rps.num_to_remove <=
           sizeof(h->rps.remove_pic) / sizeof(h->rps.remove_pic[0]));
  ...
  if (h->seq_info.b_field_coding) {
    ...
    /* reserved                  = */ u_v(bs, 1,
                                          "reserved bit for interlace coding");
  }
        ...
        if (p->wq_param == 1) {
          for (i = 0; i < 6; i++) {
            p->quant_param_undetail[i] =
                (int16_t)se_v(bs, "quant_param_delta_u") +
        ...
        if (p->wq_param == 2) {
          for (i = 0; i < 6; i++) {
            p->quant_param_detail[i] =
                (int16_t)se_v(bs, "quant_param_delta_d") +
...
/* ---------------------------------------------------------------------------
 */
static int parse_picture_header(davs2_t *h, davs2_bs_t *bs,
                                uint32_t start_code) {
  ...
  if (start_code == SC_INTRA_PICTURE) {
    ...
    if (mgr->outpics.output == -1) {
      /* An I frame is expected for the first frame or after the decoder is
       * flushed. */
  ...
  if (mgr->outpics.output != -1 &&
    ...
    davs2_log(h, DAVS2_LOG_DEBUG, "discontinuous COI (prev: %d --> curr: %d).",
  ...
  if (h->seq_info.head.low_delay == 0) {
    h->i_poc =
        h->i_coi + h->i_display_delay - h->seq_info.picture_reorder_delay;
  } else {
    h->i_poc = h->i_coi;
  }

  assert(h->i_coi >= 0 &&
         h->i_poc >= 0);  /// 'int' (2147483647) should be large enough for
                          /// 'i_coi' & 'i_poc'.
...
void parse_slice_header(davs2_t *h, davs2_bs_t *bs) {
  ...
  if (h->i_image_height > (144 * h->i_lcu_size)) {
    slice_vertical_position_extension =
        u_v(bs, 3, "slice vertical position extension");
  }
  ...
  if (h->i_width > (255 * h->i_lcu_size)) {
    slice_horizontal_positon_extension =
        u_v(bs, 2, "slice horizontal position extension");
  }
...
davs2_outpic_t *alloc_picture(int w, int h) {
  ...
  buf = (uint8_t *)davs2_malloc(
      sizeof(davs2_outpic_t) + sizeof(davs2_seq_info_t) +
 ...
 */
static INLINE int create_dpb(davs2_mgr_t *mgr) {
  davs2_seq_t *seq = &mgr->seq_info;
  uint8_t *mem_ptr = NULL;
  size_t mem_size = 0;
  int i;

  mgr->dpbsize = mgr->num_decoders + seq->picture_reorder_delay +
                 16;  /// !!! FIXME: decide dpb buffer size ?
  mgr->dpbsize += 8;  // FIXME: 需要减少

  mem_size = mgr->dpbsize * sizeof(davs2_frame_t *) +
             davs2_frame_get_size(seq->i_enc_width, seq->i_enc_height,
                                  seq->head.chroma_format, 1) *
                 mgr->dpbsize +
             davs2_frame_get_size(seq->i_enc_width, seq->i_enc_height,
                                  seq->head.chroma_format, 0) +
  ...
  for (i = 0; i < mgr->dpbsize; i++) {
    mgr->dpb[i] = davs2_frame_new(seq->i_enc_width, seq->i_enc_height,
                                  seq->head.chroma_format, &mem_ptr, 1);
    ALIGN_POINTER(mem_ptr);
...
static void init_fdec(davs2_t *h, int64_t pts, int64_t dts) {
  ...
  if (h->i_frame_type != AVS2_B_SLICE) {
    for (i = 0; i < h->num_of_references; i++) {
      h->fdec->dist_refs[i] =
          AVS2_DISTANCE_INDEX(2 * (h->fdec->i_poc - h->fref[i]->i_poc));
    ...
    h->fdec->dist_refs[B_FWD] =
        AVS2_DISTANCE_INDEX(2 * (h->fdec->i_poc - h->fref[B_FWD]->i_poc));
    h->fdec->dist_refs[B_BWD] =
        AVS2_DISTANCE_INDEX(2 * (h->fref[B_BWD]->i_poc - h->fdec->i_poc));
...
int task_decoder_update(davs2_t *h) {
  ...
  if (seq->valid_flag == 0) {
    davs2_log(h, DAVS2_LOG_ERROR,
              "failed to update decoder (invalid sequence header).");
    return -1;
  }

  if (h->b_sao != seq->enable_sao || h->b_alf != seq->enable_alf ||
      h->i_chroma_format != (int)seq->head.chroma_format ||
      h->i_lcu_level != seq->log2_lcu_size ||
      h->i_image_width != (int)seq->head.width ||
      h->i_image_height != (int)seq->head.height || h->p_integral == NULL) {
    /* resolution changed */
    ...
    if (decoder_alloc_extra_buffer(h) < 0) {
      ...
      davs2_log(h, DAVS2_LOG_ERROR,
                "failed to update the decoder(failed to alloc space).");
  ...
  if (g_bit_depth != h->sample_bit_depth) {
    davs2_log(h, DAVS2_LOG_ERROR,
              "Un-supported bit-depth %d in this version.\n",
              h->sample_bit_depth);
    return -1;
  }
#endif

  memcpy(h->wq.seq_wq_matrix, seq->seq_wq_matrix,
         2 * 64 * sizeof(int16_t)); /* weighting quantization matrix */
...
/* ---------------------------------------------------------------------------
 */
static int task_set_sequence_head(davs2_mgr_t *mgr, davs2_seq_t *seq) {
  int ret = 0;
  ...
  if (seq->valid_flag) {
    int newres = (mgr->seq_info.head.height != seq->head.height ||
                  mgr->seq_info.head.width != seq->head.width);

    memcpy(&mgr->seq_info, seq, sizeof(davs2_seq_t));

    if (newres) {
      /* resolution changed : new sequence */
      davs2_log(mgr, DAVS2_LOG_INFO, "Sequence Resolution: %dx%d.",
                seq->head.width, seq->head.height);
      if ((seq->head.width & 0) != 0 || (seq->head.height & 1) != 0) {
        davs2_log(mgr, DAVS2_LOG_ERROR,
                  "Sequence Resolution %dx%d is not even\n", seq->head.width,
                  seq->head.height);
      }
      ...
      if (create_dpb(mgr) < 0) {
        ...
        davs2_log(mgr, DAVS2_LOG_ERROR, "failed to create dpb buffers. %dx%d.",
                  seq->head.width, seq->head.height);
      }
      mgr->new_sps = TRUE;
    }
    ...
    memset(&mgr->seq_info, 0, sizeof(davs2_seq_t));
    davs2_log(mgr, DAVS2_LOG_ERROR,
              "decoded an invalid sequence header: %dx%d.", seq->head.width,
              seq->head.height);
  }
...
#define IS_VALID_FRAME(frame) \
  ((frame)->i_coi != INVALID_FRAME && (frame)->i_poc != INVALID_FRAME)
  ...
  if (h->i_frame_type == AVS2_GB_SLICE) {
    ...
    for (i = 0; i < h->rps.num_of_ref; i++) {
      ...
      if (j == mgr->dpbsize) {
        davs2_log(h, DAVS2_LOG_ERROR,
                  "reference frame of [coi: %d, poc: %d]: <COI: %d> not found.",
    ...
    if (h->i_frame_type == AVS2_B_SLICE &&
        (h->num_of_references != 2 || h->fref[0]->i_poc <= h->i_poc ||
         h->fref[1]->i_poc >= h->i_poc)) {
      davs2_log(h, DAVS2_LOG_ERROR,
                "reference frames for B frame [coi: %d, poc: %d] are wrong: %d "
                "frames found",
      ...
      if (DAVS2_ABS(frame->i_poc - h->i_poc) >= MAX_POC_DISTANCE) {
        if (frame->i_ref_count == 0) {
          davs2_log(h, DAVS2_LOG_WARNING,
                    "force to remove obsolete frame <poc: %d>.", frame->i_poc);
          ...
          if (frame->i_disposable == 0) {
            ...
            davs2_log(
                h, DAVS2_LOG_WARNING,
                "force to mark obsolete frame <poc: %d> as to be removed.",
                frame->i_poc);
          }
        }
      }
    ...
    for (;;) {
      ...
      if (open_dbp_buffer_warning) {
        davs2_log(h, DAVS2_LOG_WARNING,
                  "running out of DPB buffers, performance may suffer.");
      ...
      if (has_blocking(mgr) != 0) {
        if (mgr->outpics.pics == NULL) {
          ...
          for (i = 0; i < mgr->dpbsize; i++) {
            ...
            if (frame->i_ref_count == 0 &&
                (h->fdec == NULL || h->fdec->i_poc > frame->i_poc)) {
              if (h->fdec) {
          ...
          if (NULL == h->fdec) {
            davs2_log(h, DAVS2_LOG_ERROR,
                      "no frame for new task, DPB size (%d) too small(reorder "
                      "delay: %d) ?",
                      mgr->dpbsize, mgr->seq_info.picture_reorder_delay);
            goto fail;
          }
          ...
          davs2_log(h, DAVS2_LOG_WARNING,
                    "force one frame as the reconstruction frame.");
          ...
          /* emit an error */
          davs2_log(h, DAVS2_LOG_ERROR,
                    "the expected frame %d unavailable, proceed to frame %d.",
...
fail:
  ...
  davs2_log(NULL, DAVS2_LOG_ERROR,
            "Failed to decode frame <COI: %d, POC: %d>\n", h->i_coi, h->i_poc);
  davs2_thread_mutex_unlock(&mgr->mutex_mgr);
...
int parse_header(davs2_t *h, davs2_bs_t *p_bs) {
  ...
  while ((p_start_code = find_start_code(data + (*bitpos >> 3),
                                         len - (*bitpos >> 3))) != 0) {
    uint32_t start_code;
    *bitpos = (int)((p_start_code - data) << 3);
    ...
    switch (start_code) {
      ...
      default:
        ...
        /* using a davs2_t structure which will not be updated until a picture
         * header is decoded. */
