  ...
  for (y = 0; y < i_block_h; y++) {
    for (x = start_x; x < end_x; x += 16) {
      ...
      if (x != end_x_16) {
        ...
        mask =
            _mm_load_si128((__m128i *)(intrinsic_mask[end_x - end_x_16 - 1]));
  ...
  for (y = start_y; y < end_y; y++) {
    for (x = 0; x < i_block_w; x += 16) {
      ...
      if (x < end_x_16) {
        ...
        mask =
            _mm_loadu_si128((__m128i *)(intrinsic_mask[(i_block_w & 15) - 1]));
        _mm_maskmoveu_si128(t0, mask, (char *)(p_dst + x));
        break;
      }
    }
...
void SAO_on_block_eo_135_sse128(pel_t *p_dst, int i_dst, const pel_t *p_src,
  ...
  for (x = start_x_r0; x < end_x_r0; x += 16) {
    ...
    if (x != end_x_r0_16) {
      ...
      mask_r0 = _mm_load_si128(
          (__m128i *)(intrinsic_mask[end_x_r0 - end_x_r0_16 - 1]));
  ...
  for (y = 1; y < i_block_h - 1; y++) {
    for (x = start_x_r; x < end_x_r; x += 16) {
      ...
      if (x != end_x_r_16) {
        ...
        mask_r = _mm_load_si128(
            (__m128i *)(intrinsic_mask[end_x_r - end_x_r_16 - 1]));
  ...
  // last row
  int start_x_rn =
      lcu_avail[SAO_D] ? (lcu_avail[SAO_L] ? 0 : 1) : (i_block_w - 1);
  ...
  for (x = start_x_rn; x < end_x_rn; x += 16) {
    ...
    if (x != end_x_rn_16) {
      ...
      mask_rn = _mm_load_si128(
          (__m128i *)(intrinsic_mask[end_x_rn - end_x_rn_16 - 1]));
...
void SAO_on_block_eo_45_sse128(pel_t *p_dst, int i_dst, const pel_t *p_src,
  ...
  // first row
  int start_x_r0 =
      lcu_avail[SAO_T] ? (lcu_avail[SAO_L] ? 0 : 1) : (i_block_w - 1);
...
void SAO_on_block_bo_sse128(pel_t *p_dst, int i_dst, const pel_t *p_src,
  ...
  off1 =
      _mm_set1_epi8((int8_t)sao_param->offset[(sao_param->startBand + 1) & 31]);
  ...
  for (y = 0; y < i_block_h; y++) {
    for (x = 0; x < i_block_w; x += 16) {
      ...
      if (x < end_x_16) {
        ...
        mask = _mm_load_si128(
            (const __m128i *)intrinsic_mask[(i_block_w & 15) - 1]);
