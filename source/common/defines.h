...
#define CTRL_AEC_THREAD \
  0 /* AEC and reconstruct conducted in different threads */
...
#define AVS2_PAD \
  (64 + 16) /* number of pixels padded around the reference frame */
...
#define DAVS2_CPU_LZCNT \
  0x0000200 /* Phenom support for "leading zero count" instruction. */
#define DAVS2_CPU_AVX                                                        \
  0x0000400 /* AVX support: requires OS support even if YMM registers aren't \
               used. */
...
/* x86 modifiers */
#define DAVS2_CPU_CACHELINE_32                                                \
  0x0020000 /* avoid memory loads that span the border between two cachelines \
             */
#define DAVS2_CPU_CACHELINE_64 \
  0x0040000 /* 32/64 is the size of a cacheline in bytes */
#define DAVS2_CPU_SSE2_IS_SLOW \
  0x0080000 /* avoid most SSE2 functions on Athlon64 */
#define DAVS2_CPU_SSE2_IS_FAST \
  0x0100000 /* a few functions are only faster on Core2 and Phenom */
#define DAVS2_CPU_SLOW_SHUFFLE                                             \
  0x0200000 /* The Conroe has a slow shuffle unit (relative to overall SSE \
               performance) */
#define DAVS2_CPU_STACK_MOD4 0x0400000 /* if stack is only mod4 and not mod16 \
                                        */
#define DAVS2_CPU_SLOW_CTZ \
  0x0800000 /* BSR/BSF x86 instructions are really slow on some CPUs */
#define DAVS2_CPU_SLOW_ATOM                                             \
  0x1000000 /* The Atom is terrible: slow SSE unaligned loads, slow     \
...
#define DAVS2_CPU_FAST_NEON_MRC \
