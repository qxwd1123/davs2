...
#define DAVS2_CLIP1(a) \
  (pel_t)((a) > max_pel_value ? max_pel_value : ((a) < 0 ? 0 : (a)))

static ALWAYS_INLINE int DAVS2_CLIP3(int L, int H, int v) {
  return (((v) < (L)) ? (L) : (((v) > (H)) ? (H) : (v)));
}

#define DAVS2_SWAP(x, y) \
  {                      \
    (y) = (y) ^ (x);     \
    (x) = (y) ^ (x);     \
    (y) = (x) ^ (y);     \
...
#define CHECKED_MALLOC(var, type, size)                   \
  MULTI_LINE_MACRO_BEGIN(var) = (type)davs2_malloc(size); \
  if ((var) == NULL) {                                    \
    goto fail;                                            \
  }                                                       \
...
#pragma warning( \
    disable : 4201)  // non-standard extension used (nameless struct/union)
#endif
...
typedef struct context_set_t {
  ...
  /* 帧间预测 */
  context_t pu_type_index[NUM_INTER_DIR_CTX];  // b_pu_type_index[15] =
                                               // f_pu_type_index[3] +
                                               // dir_multi_hypothesis_mode[12]
  context_t b_pu_type_min_index[NUM_INTER_DIR_MIN_CTX];
  context_t cu_subtype_index[NUM_DIR_SKIP_CTX];  // B_Skip/B_Direct,
                                                 // F_Skip/F_Direct 公用
...
typedef struct alf_param_t {
  ...
  int coeffmulti[ALF_NUM_VARS]
                [ALF_MAX_NUM_COEF];  // 亮度分量16套，色度分量各1套
} alf_param_t;

typedef struct alf_var_t {
  alf_param_t img_param[IMG_COMPONENTS];
  int filterCoeffSym[ALF_NUM_VARS][ALF_MAX_NUM_COEF];
  int tab_region_coeff_idx[ALF_NUM_VARS]; /* coefficient look-up table for 16
                                             regions */
...
typedef struct davs2_seq_t {
  ...
  int16_t seq_wq_matrix[2]
                       [64]; /* sequence base weighting quantization matrix */
} davs2_seq_t;
...
typedef struct davs2_frame_t {
  ...
  int dist_refs[AVS2_MAX_REFS]; /* distance of reference frames, used for MV
                                   scaling */
  ...
  /* parallel */
  uint32_t i_ref_count; /* the reference count, DO NOT move its position in this
                           struct */

  int i_disposable; /* what to do with the frame when the reference count is
                       decreased to 0? */
  ...
  volatile int
      i_decoded_line; /* latest lcu line that finished reconstruction */
...
struct cu_t {
  ...
  int8_t dct_pattern[6]; /* DCT pattern of each block, dct_pattern_e, 4 luma + 2
                            chroma blocks */
...
#define PART_INDEX(w, h) \
  (g_partition_map_tab[((((w) >> 2) - 1) << 4) + ((h) >> 2) - 1])
...
struct davs2_mgr_t {
  ...
  int num_aec_thread;   /* number of threads for AEC coding (the others are for
                           reconstruction) */
...
typedef struct davs2_row_rec_t {
  ...
  bool_t b_block_avail_top;   /* availability of top  block, used in second
                                 transform */
  bool_t b_block_avail_left;  /* availability of left block, used in second
                                 transform */
  ...
  struct ctu_recon_t {
    ...
    int i_frec[3]; /* stride of reconstruction buffer (reconstruction picture)
                    */
    pel_t
        *p_frec[3]; /* reconstruction buffer pointer (reconstruction picture) */
  ...
  /* buffers */
  ALIGN32(
      pel_t buf_edge_pixels[MAX_CU_SIZE << 3]); /* intra predication buffer */
  ALIGN32(
      pel_t pred_blk[LCU_BUF_SIZE]); /* temporary buffer used for prediction */
  // ALIGN32(pel_t       fdec_buf[MAX_CU_SIZE * (MAX_CU_SIZE + (MAX_CU_SIZE >>
  // 1))]);
  struct lcu_intra_border_t {
    ALIGN32(pel_t rec_left[MAX_CU_SIZE]); /* Left border of current LCU */
    ALIGN32(pel_t rec_top[MAX_CU_SIZE * 2 +
                          32]); /* top-left, top and top-right samples
                                   (Reconstruction) of current LCU */
...
struct davs2_t {
  ...
  int output_bit_depth;     /* output bit depth (assuming: output_bit_depth <=
                               sample_bit_depth) */
  ...
  int i_slice_qp;
  int i_last_dquant;
  pel_t *intra_border[3]; /* buffer for store decoded bottom pixels of the top
                             lcu row (before filter) */
  ...
  davs2_frame_t *fdec;
  davs2_frame_t *f_background_cur; /* background reference frame, used for
                                      reconstruction */
  ...
  struct lcu_t {
    ...
    int idx_cu_zscan_aec; /* Z-scan index of current AEC CU within LCU (in 8x8
                             unit) */
    ...
    int8_t c_ipred_mode_ctx; /* context of chroma intra prediction mode (for
                                current CU decoding) */

    neighbor_inter_t
        neighbor_inter[NUM_INTER_NEIGHBOR]; /* neighboring inter modes of 4x4
                                               blocks*/
...
void avs2_trace_string2(char *trace_string, int bit_pattern, int value,
                        int len);
#endif
 ...
 * get timestamp in us
 */
static ALWAYS_INLINE int64_t davs2_get_us(void) {
#if SYS_WINDOWS
  LARGE_INTEGER nFreq;
...
static ALWAYS_INLINE int davs2_sign2(int val) { return ((val >> 31) << 1) + 1; }
 ...
 * assert
 */
#define DAVS2_ASSERT(expression, ...)              \
  if (!(expression)) {                             \
    davs2_log(NULL, DAVS2_LOG_ERROR, __VA_ARGS__); \
  }
