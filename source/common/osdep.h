...
#define ALIGN_POINTER(p)                                      \
  (p) = (uint8_t *)((intptr_t)((p) + (CACHE_LINE_SIZE - 1)) & \
                    (~(intptr_t)(CACHE_LINE_SIZE - 1)))
#define CACHE_LINE_256B 32 /* for x86-64 and x86 */
#define ALIGN_256_PTR(p)                                      \
  (p) = (uint8_t *)((intptr_t)((p) + (CACHE_LINE_256B - 1)) & \
                    (~(intptr_t)(CACHE_LINE_256B - 1)))
...
// - gcc can't align stack variables to more even if the stack were to be
// correctly aligned outside the function
// - armcc can't either, but is nice enough to actually tell you so
// - Apple gcc only maintains 4 byte alignment
// - llvm can align the stack, but only in svn and (unrelated) it exposes bugs
// in all released GNU binutils...
...
static int ALWAYS_INLINE avs2dec_pthread_create(davs2_thread_t *t, void *a,
                                                void *(*f)(void *), void *d) {
...
#elif HAVE_WIN32THREAD
#define davs2_lower_thread_priority(p) \
  SetThreadPriority(GetCurrentThread(), DAVS2_MAX(-2, -p))
#else
#define davs2_lower_thread_priority(p)
#endif
