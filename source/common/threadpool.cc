...
typedef struct davs2_sync_job_list_t {
  ...
  davs2_thread_cond_t
      cv_empty; /* event signaling that the list became emptier */
...
/* ---------------------------------------------------------------------------
 */
static int davs2_sync_job_list_init(davs2_sync_job_list_t *slist,
                                    int i_max_size) {
  if (i_max_size < 0) {
    return -1;
  }
...
/* ---------------------------------------------------------------------------
 */
static void davs2_sync_job_list_push(davs2_sync_job_list_t *slist,
...
/* ---------------------------------------------------------------------------
 */
static void *davs2_threadpool_thread(void *arg) {
...
int davs2_threadpool_init(davs2_threadpool_t **p_pool, int threads,
                          davs2_threadpool_func_t init_func, void *init_arg1,
                          int init_arg2) {
  ...
  *p_pool = pool = (davs2_threadpool_t *)mem_ptr;
  ...
  for (i = 0; i < pool->num_total_threads; i++) {
    if (davs2_thread_create(pool->thread_handle + i, NULL,
                            davs2_threadpool_thread, pool)) {
      goto fail;
    }
  }
...
/* ---------------------------------------------------------------------------
 */
void davs2_threadpool_run(davs2_threadpool_t *pool,
                          davs2_threadpool_func_t func, void *arg1, int arg2,
                          int wait_sign) {
