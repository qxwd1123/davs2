...
#define LOGI(format, ...) \
  __android_log_print(ANDROID_LOG_INFO, "davs2", format, ##__VA_ARGS__)
#endif
  ...
  if (strlen(psz_trace_file) > 0) {
    ...
    if (!h_trace) {
      ...
      davs2_log(h, DAVS2_LOG_ERROR,
                "trace: incompatible with non-regular file");
      return -1;
    }
  }
...
/* ---------------------------------------------------------------------------
 */
static ALWAYS_INLINE void davs2_set_font_color(int color) {
...
/* ---------------------------------------------------------------------------
 */
static void davs2_log_default(int i_log_level, const char *psz_fmt) {
#if !defined(_MSC_VER)
  ...
  switch (i_log_level) {
    case DAVS2_LOG_ERROR: psz_prefix = "[davs2 error]: "; break;
    case DAVS2_LOG_WARNING: psz_prefix = "[davs2 warn]: "; break;
    case DAVS2_LOG_INFO: psz_prefix = "[davs2 info]: "; break;
    case DAVS2_LOG_DEBUG: psz_prefix = "[davs2 debug]: "; break;
    default: psz_prefix = "[davs2 *]: ";
#if !defined(_MSC_VER)
...
void davs2_log(void *handle, int level, const char *format, ...) {
  ...
  DAVS2_ASSERT(level >= 0 && level < DAVS2_LOG_MAX, "Invalid log level %d",
               level);
